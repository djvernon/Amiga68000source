...
* z rotate, no zoom
auto2	st.b	frames.requested
	move.w	base.z.angle(pc),d1
	cmp.w	#$400,d1
	blt.s	auto.update
	addq.w	#1,sequence.no
	move.l	#-Z_STEP,zstep
...
length\1
	...
	IFD	NEW_METHOD
; texture data - long per pixel
	move.l	(a0),d0		pixel 1

	add.l	d0,d0
	or.l	(a0),d0		pixels 2-8
...
; do one less pixel in span - don't do 33 pixels
...
* d4.l = txstart, d5.w = tystart
* d0.l = txend, d1.w = tyend
* d6.w = length (always greater than zero)
calc.increments
...
fill.width.table
	dc.l	length1,length2,length3,length4
	dc.l	length5,length6,length7,length8
	dc.l	length9,length10,length11,length12
	dc.l	length13,length14,length15,length16
	dc.l	length17,length18,length19,length20
	dc.l	length21,length22,length23,length24
	dc.l	length25,length26,length27,length28
	dc.l	length29,length30,length31,length32
...
fpsprint
	move.l	#156250,d2		pulses per second * 10
	divu	d1,d2			frames per second * 10
	move.w	oldfps(pc),d0
	cmp.w	d0,d2
	bge.s	md
	move.w	d2,oldfps
