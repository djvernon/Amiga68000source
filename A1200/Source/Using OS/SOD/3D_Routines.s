...
_3D_VIEW_WIDTH	equ	320
_3D_VIEW_HEIGHT	equ	200
_3D_VIEW_DEPTH	equ	4

PALETTE_WIDTH	equ	_3D_VIEW_WIDTH
PALETTE_HEIGHT	equ	256-_3D_VIEW_HEIGHT

_3D_VIEW_MEMORY	equ	_3D_VIEW_WIDTH/8*_3D_VIEW_HEIGHT*_3D_VIEW_DEPTH*2
PALETTE_MEMORY	equ	PALETTE_WIDTH/8*PALETTE_HEIGHT*_3D_VIEW_DEPTH
TOTAL_MEMORY	equ	_3D_VIEW_MEMORY+PALETTE_MEMORY


****************************************


create.3D.view.screen

* using graphics primitives, create a view that can be loaded on demand
*
* returns 1 if view was created successfully
* returns -1 otherwise

	move.l	#TOTAL_MEMORY,d0
	move.l	#MEMF_CHIP!MEMF_CLEAR,d1
	CALLEXEC AllocMem
	move.l	d0,_3D.view.screen.memory
	beq	.create.failure

	move.l	d0,_3D.view.screen1
	add.l	#_3D_VIEW_WIDTH/8*_3D_VIEW_HEIGHT*_3D_VIEW_DEPTH,d0
	move.l	d0,_3D.view.screen2
	add.l	#_3D_VIEW_WIDTH/8*_3D_VIEW_HEIGHT*_3D_VIEW_DEPTH,d0
	move.l	d0,palette.screen
...
* initialise view
	...
	lea	v(pc),a1
	move.l	#vp1,v_ViewPort(a1)

* initialise colormap

	move.l	#1<<_3D_VIEW_DEPTH,d0	number of colour entries
	CALLGRAF GetColorMap
	move.l	d0,cm
	beq	.create.failure.free.mem

	lea	_3D.view.colour.table(pc),a0
	move.l	d0,a1
	move.l	cm_ColorTable(a1),a1
	REPT	(1<<_3D_VIEW_DEPTH)-1
	move.w	(a0)+,(a1)+
	ENDR
	move.w	(a0),(a1)

* initialise viewport for 3D view

	lea	vp1(pc),a0
	CALLGRAF InitVPort

	lea	vp1(pc),a1
	move.l	#vp2,vp_Next(a1)
	move.l	cm(pc),vp_ColorMap(a1)
	move.w	#_3D_VIEW_WIDTH,vp_DWidth(a1)
	move.w	#_3D_VIEW_HEIGHT,vp_DHeight(a1)
	move.l	#ri1,vp_RasInfo(a1)

* initialise viewport for palette
	...
	lea	vp2(pc),a1
	move.l	cm(pc),vp_ColorMap(a1)
	move.w	#PALETTE_WIDTH,vp_DWidth(a1)
	move.w	#PALETTE_HEIGHT,vp_DHeight(a1)
	move.w	#_3D_VIEW_HEIGHT+2,vp_DyOffset(a1)
	move.l	#ri2,vp_RasInfo(a1)

* initialise rasinfo with pointer to first 3D bitmap

	lea	ri1(pc),a1
	move.l	#bm1,ri_BitMap(a1)

* initialise rasinfo with pointer to palette bitmap

	lea	ri2(pc),a1
	move.l	#bm3,ri_BitMap(a1)

* initialise first 3D bitmap

	lea	bm1(pc),a0
	moveq	#_3D_VIEW_DEPTH,d0
	move.l	#_3D_VIEW_WIDTH,d1
	move.l	#_3D_VIEW_HEIGHT,d2
	CALLGRAF InitBitMap

	move.l	_3D.view.screen1(pc),a0
	lea	bm1(pc),a1
	lea	bm_Planes(a1),a1
	REPT	_3D_VIEW_DEPTH-1
	move.l	a0,(a1)+
	lea	_3D_VIEW_WIDTH/8*_3D_VIEW_HEIGHT(a0),a0
	ENDR
	move.l	a0,(a1)

* initialise palette bitmap

	lea	bm3(pc),a0
	moveq	#_3D_VIEW_DEPTH,d0
	move.l	#PALETTE_WIDTH,d1
	moveq	#PALETTE_HEIGHT,d2
	CALLGRAF InitBitMap

	move.l	palette.screen(pc),a0
	lea	bm3(pc),a1
	lea	bm_Planes(a1),a1
	REPT	_3D_VIEW_DEPTH-1
	move.l	a0,(a1)+
	lea	PALETTE_WIDTH/8*PALETTE_HEIGHT(a0),a0
	ENDR
	move.l	a0,(a1)

* initialise rastport for palette

	lea	rp(pc),a1
	CALLGRAF InitRastPort

	lea	rp(pc),a1
	move.l	#bm3,rp_BitMap(a1)

* construct intermediate copper list for viewport, first 3D bitmap

	lea	v(pc),a0
	lea	vp1(pc),a1
	CALLGRAF MakeVPort

* construct intermediate copper list for viewport, palette bitmap

	lea	v(pc),a0
	lea	vp2(pc),a1
	CALLGRAF MakeVPort

* merge all copper lists together into a single list, first 3D bitmap

	lea	v(pc),a1
	CALLGRAF MrgCop

* save LOFCprList for first 3D bitmap, then reset to zero
...
* initialise rasinfo with pointer to second 3D bitmap
...
* construct intermediate copper list for viewport, second 3D bitmap

	lea	v(pc),a0
	lea	vp1(pc),a1
	CALLGRAF MakeVPort

* merge all copper lists together into a single list, second 3D bitmap

	lea	v(pc),a1
	CALLGRAF MrgCop

* save LOFCprList for second 3D bitmap

	lea	v(pc),a1
	lea	v_LOFCprList(a1),a1
	move.l	(a1),copper2

* draw colour palette boxes

	bsr	draw.3D.view.screen.palette
...
* add reply port for input device

	lea	input.device.reply.port(pc),a1
...
* open input device
	...
	lea	input.device.name(pc),a0
	moveq	#0,d0
	lea	input.IO.request(pc),a1
	moveq	#0,d1
	CALLEXEC OpenDevice
	tst.l	d0
	bne.s	.create.failure.remove.port

* add input handler with priority 52

	lea	input.handler.interrupt(pc),a0
	move.b	#NT_INTERRUPT,LN_TYPE(a0)
	move.b	#52,LN_PRI(a0)
	move.l	#input.handler.interrupt.name,LN_NAME(a0)
	move.l	#input.handler.interrupt.code,IS_CODE(a0)

	lea	input.IO.request(pc),a1
	move.l	#input.device.reply.port,MN_REPLYPORT(a1)
...
.create.failure.remove.port
	lea	input.device.reply.port(pc),a1
	CALLEXEC RemPort

.create.failure.free.mem
	move.l	_3D.view.screen.memory(pc),a1
	move.l	#TOTAL_MEMORY,d0
	CALLEXEC FreeMem
...
destroy.3D.view.screen
	lea	input.IO.request(pc),a1
	move.w	#IND_REMHANDLER,IO_COMMAND(a1)
	move.l	#input.handler.interrupt,IO_DATA(a1)
	CALLEXEC DoIO
...
****************************************
	...
	IFEQ	(_3D_VIEW_DEPTH-4)
PALETTE_COLUMNS	equ	16		* for 16 colour screen
PALETTE_ROWS	equ	1
	ENDC

	IFEQ	(_3D_VIEW_DEPTH-5)
PALETTE_COLUMNS	equ	16		* for 32 colour screen
PALETTE_ROWS	equ	2
	ENDC

	IFEQ	(_3D_VIEW_DEPTH-6)
PALETTE_COLUMNS	equ	16		* for 64 colour screen
PALETTE_ROWS	equ	4
	ENDC

	IFEQ	(_3D_VIEW_DEPTH-7)
PALETTE_COLUMNS	equ	32		* for 128 colour screen
PALETTE_ROWS	equ	4
	ENDC

	IFEQ	(_3D_VIEW_DEPTH-8)
PALETTE_COLUMNS	equ	32		* for 256 colour screen
PALETTE_ROWS	equ	8
	ENDC


BOX_WIDTH	equ	PALETTE_WIDTH/PALETTE_COLUMNS
BOX_HEIGHT	equ	PALETTE_HEIGHT/PALETTE_ROWS


draw.3D.view.screen.palette
	clr.w	palette.colour

	moveq	#0,d1
	move.w	#BOX_HEIGHT-1,d3

	move.w	#PALETTE_ROWS-1,d7
.row.loop
	moveq	#0,d0
	move.w	#BOX_WIDTH-1,d2

	move.w	#PALETTE_COLUMNS-1,d6
.column.loop
	movem.l	d0-d3,-(sp)
	move.w	palette.colour(pc),d0
	lea	rp(pc),a1
	CALLGRAF SetAPen
	movem.l	(sp)+,d0-d3
...
calc.y.tab
	move.w	#_3D_VIEW_HEIGHT-1,d0
	moveq	#0,d1			offset starts at zero
	move.w	#_3D_VIEW_WIDTH/8,d2	width of one bitplane
	lea	y.table(pc),a0
...
* returns 1 if view shown successfully
* returns -1 otherwise

* open dummy intuition screen to provide mouse co-ordinates

	lea	new.dummy.3D.view.screen(pc),a0
	CALLINT	OpenScreen
	move.l	d0,dummy.3D.view.screen
	beq.s	.show.failure

* save normal view to restore later

	move.l	_GfxBase(pc),a0
	move.l	gb_ActiView(a0),normal.view
...
;	CALLGRAF OwnBlitter
	...
	move.l	4.w,a6			division-by-zero exception handler
	move.l	ThisTask(a6),a6
	move.l	TC_TRAPCODE(a6),old.dbz
	move.l	#dbz.code,TC_TRAPCODE(a6)

	sf	left.button
	sf	right.button
	sf	vblank.occured
	sf	_3D.view.quit.request
	st	_3D.view.active
...
restore.normal.screen
	...
	move.l	4.w,a6			restore division-by-zero handler
	move.l	ThisTask(a6),a6
	move.l	old.dbz,TC_TRAPCODE(a6)

;	CALLGRAF DisownBlitter

	move.l	dummy.3D.view.screen(pc),a0
	CALLINT	CloseScreen

* restore the normal view

	move.l	normal.view(pc),a1
	CALLGRAF LoadView
	rts
...
input.handler.interrupt.code
	move.l	a0,-(sp)

	move.b	_3D.view.active(pc),d0
	beq.s	exit.input.handler.interrupt
...
.left.down
	...
	move.l	dummy.3D.view.screen(pc),a0
	move.w	sc_MouseX(a0),colour.x
	move.w	sc_MouseY(a0),colour.y
	bra.s	exit.input.handler.interrupt
...
.right.up
	cmp.w	#IECODE_RBUTTON!IECODE_UP_PREFIX,d0
	bne.s	exit.input.handler.interrupt
	sf	right.button
	bra.s	exit.input.handler.interrupt
...
dbz.code
	cmp.l	#5,(sp)
	beq.s	.dbz			if division-by-zero

	move.l	old.dbz(pc),a0
	jmp	(a0)			else use system handler
...
_3D.view.screen.memory	dc.l	0
_3D.view.screen1	dc.l	0
_3D.view.screen2	dc.l	0
palette.screen	dc.l	0
cm	dc.l	0
dummy.3D.view.screen	dc.l	0
normal.view	dc.l	0
old.dbz	dc.l	0
copper1	dc.l	0
copper2	dc.l	0
...
palette.colour	dc.w	0

input.handler.interrupt	ds.b	IS_SIZE
input.device.reply.port	ds.b	MP_SIZE
input.IO.request	ds.b	IOSTD_SIZE

_3D.view.active	dc.b	0
left.button	dc.b	0
right.button	dc.b	0
vblank.occured	dc.b	0
	even
...
vblank.name	dc.b	'SOD VBlank',0
	even

input.device.name	dc.b	'input.device',0
input.handler.interrupt.name	dc.b	'SOD InputHandler',0
	even
...
new.dummy.3D.view.screen
		dc.w	0,_3D_VIEW_HEIGHT+2	left, top
		dc.w	PALETTE_WIDTH,PALETTE_HEIGHT
		dc.w	1		depth
		dc.b	0,1		pens
		dc.w	0		viewmodes
		dc.w	SCREENBEHIND!CUSTOMSCREEN	type
		dc.l	0		font
		dc.l	0		title
		dc.l	0		gadgets
		dc.l	0		bitmap
...
XMAX	equ	_3D_VIEW_WIDTH
YMAX	equ	_3D_VIEW_HEIGHT
XMID	equ	XMAX/2
YMID	equ	YMAX/2


clear.current.screen
	move.l	_3D.view.screen1(pc),a2
	move.w	#(YMAX*4)-1,d2
	moveq	#0,d3
.loop
	REPT	XMAX/32
...
no.sign	add.w	d2,d2			2 Ldelta
	...
	move.l	#_3D_VIEW_WIDTH/8*_3D_VIEW_HEIGHT,d5	size of one bitplane
...
bltfin	btst	#6,dmaconr(a6)
	...
	moveq	#_3D_VIEW_WIDTH/8,d3	width of one bitplane
...
fill.loop
	...
	lea	_3D_VIEW_WIDTH/8*_3D_VIEW_HEIGHT(a2),a3		bitplane 2
	lea	_3D_VIEW_WIDTH/8*_3D_VIEW_HEIGHT(a3),a4		bitplane 3
	lea	_3D_VIEW_WIDTH/8*_3D_VIEW_HEIGHT(a4),a5		bitplane 4
...
next.line
	lea	_3D_VIEW_WIDTH/8(a0),a0	next line
...
y.table	ds.w	_3D_VIEW_HEIGHT



fill.coords
	ds.w	_3D_VIEW_HEIGHT*2+2



poly.coords	ds.w	MAX_POLY_SIDES2*4
...
update.visible.screen
	move.l	_3D.view.screen1(pc),d0
	move.l	_3D.view.screen2(pc),_3D.view.screen1
	move.l	d0,_3D.view.screen2
...
_3D_ROTATE	equ	rotate.coords-line
_3D_POLYDRAW	equ	polygon.draw-line
_3D_POLYGON	equ	polygon.orientation-line
_3D_GOSUB	equ	sub.object-line
_3D_END		equ	end.draw.3D.object-line
_3D_FILLCOL	equ	set.fill.colour-line
_3D_VECTCOL	equ	set.line.colour-line
_3D_VECTOR	equ	0
_3D_CIRCLE	equ	circle-line
_3D_ZPRI	equ	component.priority-line
_3D_GOTO	equ	skip.data-line


object	dc.w	_3D_ROTATE
