...
; 2. Combine the four planex_data areas into one so that plot_fill_dots only has to be called once and therefore only
;    needs to calculate each x once (still need four blitter fills though because only writing to portion of screen)
; 3. Optimise plot_fill_dots (use dbra count for the loop instead of doing "bmi.s .next_x" each time and also unroll the loop)


HORIZ_SCALE_ONLY	equ	1	; only do horizontal sprite scaling

; Only uncomment one of the following lines
;TEST_CHUNKY_METHOD	equ	1	; Real A500 5.7 fps
TEST_TRODDLERS_METHOD	equ	1	; Real A500 27-28 fps
...
; e.g. move blit_wait to here?
	...
	lea	plane3_data,a1
	lea	plane3_fill_source,a2
	bsr	plot_fill_dots
	bsr	blit_wait
	bsr	fill_plane3

	lea	plane4_data,a1
	lea	plane4_fill_source,a2
	bsr	plot_fill_dots
	bsr	blit_wait
	bra	fill_plane4
...
; processes row from right to left (same direction as blitter fill)
	...
	move.l	a1,a0		; store current row count address for later
	addq.w	#2,a1
...
fill_plane1
	lea	plane1_fill_source+(SOURCE_HEIGHT*BYTES_PER_LINE)-2,a0
	move.l	screen1,a1
...
fill_plane2
	lea	plane2_fill_source+(SOURCE_HEIGHT*BYTES_PER_LINE)-2,a0
	move.l	screen1,a1
...
fill_plane3
	lea	plane3_fill_source+(SOURCE_HEIGHT*BYTES_PER_LINE)-2,a0
	move.l	screen1,a1
; point to last word of bitplane 3's target area
	add.l	#(((SCREEN_HEIGHT-SOURCE_HEIGHT)/2)+SOURCE_HEIGHT+SCREEN_HEIGHT*2)*BYTES_PER_LINE-2,a1
	bra.s	fill_plane

fill_plane4
	lea	plane4_fill_source+(SOURCE_HEIGHT*BYTES_PER_LINE)-2,a0
	move.l	screen1,a1
; point to last word of bitplane 4's target area
	add.l	#(((SCREEN_HEIGHT-SOURCE_HEIGHT)/2)+SOURCE_HEIGHT+SCREEN_HEIGHT*3)*BYTES_PER_LINE-2,a1
...
plane1_fill_source
	ds.w	SOURCE_HEIGHT*WORDS_PER_LINE
plane2_fill_source	; must immediately follow plane1_fill_source
	ds.w	SOURCE_HEIGHT*WORDS_PER_LINE
plane3_fill_source	; must immediately follow plane2_fill_source
	ds.w	SOURCE_HEIGHT*WORDS_PER_LINE
plane4_fill_source	; must immediately follow plane3_fill_source
	ds.w	SOURCE_HEIGHT*WORDS_PER_LINE
...
plane1_definition
	...
	ds.w	(SOURCE_WIDTH+1)*SOURCE_HEIGHT	; worst case
; plane 2
plane2_definition
	ds.w	1	; number of rows
	ds.w	(SOURCE_WIDTH+1)*SOURCE_HEIGHT	; worst case
...
plane1_data
	ds.w	((1+SOURCE_HEIGHT+1)*(SOURCE_WIDTH+1))+1	; worst case
plane2_data
	ds.w	((1+SOURCE_HEIGHT+1)*(SOURCE_WIDTH+1))+1	; worst case
plane3_data
	ds.w	((1+SOURCE_HEIGHT+1)*(SOURCE_WIDTH+1))+1	; worst case
