...
* Save vertical's rotated co-ords
	...
	add.w	#CHUNKY_SCREEN_WIDTH/2,d0	centre on screen
...
calc.wall.strip.offsets
	...
	sub.w	#CHUNKY_SCREEN_WIDTH/2,d6
	ext.l	d6
...
strip.data
	ds.w	CHUNKY_SCREEN_WIDTH		source offset
...
*	68000 CPU cycles required: 100 + 578 per loop (outputting 32 pixels) + 16
*	= (100 + ((320*200)/32 * 578) + 16) = 1156116 cycles for 320*200 display
*	approx. 18 CPU cycles per output pixel

* For rotate method:-
*	68000 CPU cycles required: 100 + 522 per loop (outputting 32 pixels) + 16
*	= (100 + ((320*200)/32 * 522) + 16) = 1044116 cycles for 320*200 display
*	approx. 16 CPU cycles per output pixel
...
c2p.double
	...
	move.l	#$00ff00ff,d6				12
	move.l	#$0f0f0f0f,a6				12
	move.l	#$33333333,a5				12
...
* 16-bit transpose ABCD and IJKL
	swap	d2			KLIJ		4
	move.w	d0,d4			..CD		4
	move.w	d2,d0			ABIJ		4
	move.w	d4,d2			KLCD		4
	swap	d2			CDKL		4 = total 20 cycles

* 16-bit transpose EFGH and MNOP
	swap	d3			OPMN
	move.w	d1,d4			..GH
	move.w	d3,d1			EFMN
	move.w	d4,d3			OPGH
	swap	d3			GHOP		total 20 cycles
...
* 8-bit transpose ABIJ and EFMN
*			d0 = ABIJ
*			d1 = EFMN
	move.l	d1,d4						4
	lsr.l	#8,d4	d4 = .EFM				24
	eor.l	d0,d4						8
	and.l	d6,d4	d4 = (d4 ^ d0) & $00ff00ff = mask	8
	eor.l	d4,d0	d0 = AEIM				8
	lsl.l	#8,d4	d4 = mask << 8				24
	eor.l	d4,d1	d1 = BFJN				8 = total 84 cycles

* 8-bit transpose CDKL and GHOP
*			d2 = CDKL
*			d3 = GHOP
	move.l	d3,d4
	lsr.l	#8,d4	d4 = .GHO
	eor.l	d2,d4
	and.l	d6,d4	d4 = (d4 ^ d2) & $00ff00ff = mask
	eor.l	d4,d2	d2 = CGKO
	lsl.l	#8,d4	d4 = mask << 8
	eor.l	d4,d3	d3 = DHLP				total 84 cycles
...
* 4-bit transpose AEIM and CGKO
*			d0 = A3A2A1A0E3E2E1E0I3I2I1I0M3M2M1M0 (Note that each symbol, e.g. 'A3', represents two bits)
*			d2 = C3C2C1C0G3G2G1G0K3K2K1K0O3O2O1O0
	move.l	d2,d4						4
	lsr.l	#4,d4	d4 = ....C3C2C1C0G3G2G1G0K3K2K1K0O3O2	16
	eor.l	d0,d4						8
	and.l	d5,d4	d4 = (d4 ^ d0) & $0f0f0f0f = mask	8
	eor.l	d4,d0	d0 = A3A2C3C2E3E2G3G2I3I2K3K2M3M2O3O2	8
	lsl.l	#4,d4	d4 = mask << 4				16
	eor.l	d4,d2	d2 = A1A0C1C0E1E0G1G0I1I0K1K0M1M0O1O0	8 = total 68 cycles

* 4-bit transpose BFJN and DHLP
*			d1 = B3B2B1B0F3F2F1F0J3J2J1J0N3N2N1N0
*			d3 = D3D2D1D0H3H2H1H0L3L2L1L0P3P2P1P0
	move.l	d3,d4
	lsr.l	#4,d4	d4 = ....D3D2D1D0H3H2H1H0L3L2L1L0P3P2
	eor.l	d1,d4
	and.l	d5,d4	d4 = (d4 ^ d0) & $0f0f0f0f = mask
	eor.l	d4,d1	d1 = B3B2D3D2F3F2H3H2J3J2L3L2N3N2P3P2
	lsl.l	#4,d4	d4 = mask << 4
	eor.l	d4,d3	d3 = B1B0D1D0F1F0H1H0J1J0L1L0N1N0P1P0	total 68 cycles

	move.l	a5,d5			4

* 2-bit transpose and output bits 0 and 1
*			d2 = A1A0C1C0E1E0G1G0I1I0K1K0M1M0O1O0
*			d3 = B1B0D1D0F1F0H1H0J1J0L1L0N1N0P1P0
	move.l	d3,d4						4
	lsr.l	#2,d4	d4 = ..B1B0D1D0F1F0H1H0J1J0L1L0N1N0P1	12
	eor.l	d2,d4						8
	and.l	d5,d4	d4 = (d4 ^ d0) & $33333333 = mask	8
	eor.l	d4,d2	d2 = A1B1C1D1E1F1G1H1I1J1K1L1M1N1O1P1	8
	move.l	d2,(a3)+		plane 1			12
	lsl.l	#2,d4	d4 = mask << 2				12
	eor.l	d4,d3	d3 = A0B0C0D0E0F0G0H0I0J0K0L0M0N0O0P0	8
	move.l	d3,(a4)+		plane 0			12 = total 84 cycles

* 2-bit transpose and output bits 2 and 3
*			d0 = A3A2C3C2E3E2G3G2I3I2K3K2M3M2O3O2
*			d1 = B3B2D3D2F3F2H3H2J3J2L3L2N3N2P3P2
	move.l	d1,d4
	lsr.l	#2,d4	d4 = ..B3B2D3D2F3F2H3H2J3J2L3L2N3N2P3
	eor.l	d0,d4
	and.l	d5,d4	d4 = (d4 ^ d0) & $33333333 = mask
	eor.l	d4,d0	d0 = A3B3C3D3E3F3G3H3I3J3K3L3M3N3O3P3
	move.l	d0,(a1)+		plane 3
	lsl.l	#2,d4	d4 = mask << 2
	eor.l	d4,d1	d1 = A2B2C2D2E2F2G2H2I2J2K2L2M2N2O2P2
	move.l	d1,(a2)+		plane 2			total 84 cycles
...
*			d1 = EFMN
	move.l	d0,d4						4
	ror.l	#8,d1	d1 = NEFM				24
	eor.l	d1,d4						8
	and.l	d6,d4	d4 = (d4 ^ d1) & $00ff00ff = mask	8
	eor.l	d4,d0	d0 = AEIM				8
	eor.l	d4,d1	d1 = NBFJ				8 = total 60 cycles
...
*			d3 = GHOP
	...
	ror.l	#8,d3	d3 = PGHO
	eor.l	d3,d4
	and.l	d6,d4	d4 = (d4 ^ d3) & $00ff00ff = mask
	eor.l	d4,d2	d2 = CGKO
	eor.l	d4,d3	d3 = PDHL				total 60 cycles
...
*			d2 = C3C2C1C0G3G2G1G0K3K2K1K0O3O2O1O0
	move.l	d0,d4						4
	ror.l	#4,d2	d2 = O1O0C3C2C1C0G3G2G1G0K3K2K1K0O3O2	16
	eor.l	d2,d4						8
	and.l	d5,d4	d4 = (d4 ^ d2) & $0f0f0f0f = mask	8
	eor.l	d4,d0	d0 = A3A2C3C2E3E2G3G2I3I2K3K2M3M2O3O2	8
	eor.l	d4,d2	d2 = O1O0A1A0C1C0E1E0G1G0I1I0K1K0M1M0	8 = total 52 cycles

* 4-bit transpose NBFJ and PDHL
*			d1 = N3N2N1N0B3B2B1B0F3F2F1F0J3J2J1J0
*			d3 = P3P2P1P0D3D2D1D0H3H2H1H0L3L2L1L0
	rol.l	#4,d1	d1 = N1N0B3B2B1B0F3F2F1F0J3J2J1J0N3N2
	move.l	d1,d4
	eor.l	d3,d4
	and.l	d5,d4	d4 = (d4 ^ d3) & $0f0f0f0f = mask
	eor.l	d4,d1	d1 = N1N0P1P0B1B0D1D0F1F0H1H0J1J0L1L0
	eor.l	d4,d3	d3 = P3P2B3B2D3D2F3F2H3H2J3J2L3L2N3N2	total 52 cycles

	move.l	a5,d5			4

* 2-bit transpose and output bits 0 and 1
*			d2 = O1O0A1A0C1C0E1E0G1G0I1I0K1K0M1M0
*			d1 = N1N0P1P0B1B0D1D0F1F0H1H0J1J0L1L0
	rol.l	#2,d1	d1 = N0P1P0B1B0D1D0F1F0H1H0J1J0L1L0N1	12
	move.l	d1,d4						4
	eor.l	d2,d4						8
	and.l	d5,d4	d4 = (d4 ^ d2) & $33333333 = mask	8
	eor.l	d4,d2	d2 = O1P1A1B1C1D1E1F1G1H1I1J1K1L1M1N1	8
	rol.l	#4,d2						16
	move.l	d2,(a3)+		plane 1			12
	eor.l	d4,d1	d1 = N0O0P0A0B0C0D0E0F0G0H0I0J0K0L0M0	8
	rol.l	#6,d1						20
	move.l	d1,(a4)+		plane 0			12 = total 108 cycles

* 2-bit transpose and output bits 2 and 3
*			d0 = A3A2C3C2E3E2G3G2I3I2K3K2M3M2O3O2
*			d3 = P3P2B3B2D3D2F3F2H3H2J3J2L3L2N3N2
	rol.l	#2,d3	d3 = P2B3B2D3D2F3F2H3H2J3J2L3L2N3N2P3	12
	move.l	d3,d4						4
	eor.l	d0,d4						8
	and.l	d5,d4	d4 = (d4 ^ d0) & $33333333 = mask	8
	eor.l	d4,d0	d0 = A3B3C3D3E3F3G3H3I3J3K3L3M3N3O3P3	8
	move.l	d0,(a1)+		plane 3			12
	eor.l	d4,d3	d3 = P2A2B2C2D2E2F2G2H2I2J2K2L2M2N2O2	8
	rol.l	#2,d3						12
	move.l	d3,(a2)+		plane 2			12 = total 84 cycles
	ENDC

* TO DO: Try to further separate the plane 1/0 and plane 3/2 memory writes above, to improve performance on 68020+
* (e.g. by using d1 and d2 which are spare after plane 1 and 0 are written)
