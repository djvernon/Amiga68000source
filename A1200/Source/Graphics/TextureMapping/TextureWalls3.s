...
* d5 = x, d6 = y1, d7 = y2
* a4 = chunky data address
* uses d4-d7, a5

draw.strip
	move.l	chunky.memory(pc),a5
	sub.w	d6,d7
	mulu	#SCREEN_WIDTH,d6
	add.l	d6,a5
	add.w	d5,a5			screen address

;	move.w	d7,raster.count

	move.l	#(SOURCE_HEIGHT-1)<<16,d5
	divul.l	d7,d5:d5
	moveq	#0,d6

.loop	swap	d6
	move.w	d6,d4
	mulu	#SOURCE_WIDTH,d4
	swap	d6
	add.l	d5,d6

	move.b	(a4,d4.l),(a5)
	lea	SCREEN_WIDTH(a5),a5
	dbra	d7,.loop
	rts
...
frames.per.sec			; using horiz. sync. pulse counter in CIA-B
	...
	move.b	$bfda00.l,d0		get counter into latch
	move.b	$bfd900.l,d0		bits 8-15 of counter
	lsl.w	#8,d0			into correct position
	move.b	$bfd800.l,d0		bits 0-7 of counter
