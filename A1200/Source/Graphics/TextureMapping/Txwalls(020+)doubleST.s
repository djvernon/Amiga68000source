...
* Allocate Atari ST intermediate screen memory

	move.l	#PLANAR_SCREEN_SIZE,d0
	moveq	#2,d1			chip
	jsr	-198(a6)		AllocMem
	move.l	d0,st.screen.memory
	beq	exit_free_mem
...
c2p.double.st
	...
	move.l	st.screen.memory(pc),a1			16
...
.nextp0	moveq	#0,d1		pixel 1

.nextp1	move.l	(a0,d0.w),d2
	or.l	(a1,d1.w),d2
...
c2p.alive
	...
	move.l	st.screen.memory(pc),a5       ;the target		16
...
* Routine to convert from Atari ST screen format (interleaved bitplane words) to Amiga format (separate bitplanes)
st.screen.to.amiga
	move.l	st.screen.memory(pc),a0
	...
	move.w	#(SCREEN_WIDTH*SCREEN_HEIGHT)/16-1,d7	8
...
* 2-dimensional arrays for converting two pixels at once
cd2dp_01	ds.l	64*64
cd2dp_23	ds.l	64*64
