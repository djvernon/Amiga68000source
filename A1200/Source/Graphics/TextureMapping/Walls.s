...
main.loop
	bsr	read.joystick
	bsr	player.position
	bsr	player.direction
	bsr	player.visibility

;	movem.w	player.x.angle(pc),d0-d2	get x angle, y angle and z angle
;	bsr	calc.sin.cos.values

	bsr	draw.overhead.view
...
set.x.y.z.angles
	...
	add.w	d0,player.z.angle	update z angle
	and.w	#$7fe,player.z.angle

	move.b	(a0),d0			mouse y
	ext.w	d0
	add.w	d0,d0			word offset for tables
	add.w	d0,player.x.angle	update x angle
	and.w	#$7fe,player.x.angle
	rts
...
right.pressed
	...
	ext.w	d0
	add.w	d0,d0			word offset for tables
	add.w	d0,player.y.angle	update y angle
...
.store	move.w	d2,joy.x.speed
	move.w	d3,joy.y.speed
...
MAX_LEVEL_WIDTH	equ	32
MAX_LEVEL_DEPTH	equ	32

SQUARE_SIZE	equ	$8000000
OFFSET_MASK	equ	$7ffffff
OFFSET_SHIFT	equ	27


initialise.level
	move.w	#8,level.width
	move.w	#6,level.depth

	move.l	#8*SQUARE_SIZE,level.x.max
	move.l	#6*SQUARE_SIZE,level.z.max

	move.l	#0,player.x
	move.l	#0,player.z

	bsr	make.level.coords
	bsr	make.level.x.walls
	bsr	make.level.z.walls
	rts
...
make.level.coords
	lea	level.coords(pc),a5
	move.l	#SQUARE_SIZE,d7

	move.w	level.depth(pc),d6
	moveq	#0,d5

.depth	move.w	level.width(pc),d4
	moveq	#0,d3

.width	move.l	d3,(a5)+		store x
	move.l	d5,(a5)+		store z
	add.l	d7,d3
	dbra	d4,.width
...
level.coords				* View from below
	ds.l	2*(MAX_LEVEL_WIDTH+1)*(MAX_LEVEL_DEPTH+1)


make.level.x.walls
	lea	x.walls.map(pc),a5
	lea	x.walls(pc),a4
	moveq	#0,d7

	move.w	level.depth(pc),d6
	move.w	level.width(pc),d5
	move.w	d5,d4
	add.w	d4,d4
	mulu	d6,d5
	lea	(a5,d5.l),a5

.depth	move.w	level.width(pc),d5
	subq.w	#1,d5

.width	move.w	d7,(a4)+		store point no.

	cmp.b	#'-',(a5)+
	bne.s	.gap

.wall	move.w	#-1,(a4)+		store wall flag
	bra.s	.next

.gap	move.w	#0,(a4)+		store gap

.next	addq.w	#1,d7			next point no.
	dbra	d5,.width

	move.w	d7,(a4)+		store point no.
	addq.w	#1,d7			next point no.
...
make.level.z.walls			* Not reversed yet
	lea	z.walls.map(pc),a5
	lea	z.walls(pc),a4
	moveq	#0,d7
...
x.walls					* View from below
	ds.w	((MAX_LEVEL_WIDTH*2)+1)*(MAX_LEVEL_DEPTH+1)

z.walls					* View from below
	ds.w	((MAX_LEVEL_WIDTH+1)*2+1)*(MAX_LEVEL_DEPTH)
...
.x	move.w	player.x(pc),d0		update player's x position
	move.w	joy.x.speed(pc),d1
	asl.w	#4,d1
	add.w	d1,d0
	bpl.s	.x.plus
	moveq	#0,d0
...
.x.plus	cmp.w	level.x.max(pc),d0
	blt.s	.x.ok
	move.w	level.x.max(pc),d0
	subq.w	#1,d0

.x.ok	move.w	d0,player.x

.z	move.w	player.z(pc),d0		update player's z position
	move.w	joy.y.speed(pc),d1
	asl.w	#4,d1
	add.w	d1,d0
	bpl.s	.z.plus
	moveq	#0,d0
...
.z.plus	cmp.w	level.z.max(pc),d0
	blt.s	.z.ok
	move.w	level.z.max(pc),d0
	subq.w	#1,d0

.z.ok	move.w	d0,player.z
	bra.s	.calc

.fire	move.w	joy.x.speed(pc),d0	update player's y angle
	add.w	d0,player.y.angle
	and.w	#$7fe,player.y.angle

.calc	moveq	#OFFSET_SHIFT,d5

	move.l	player.x(pc),d7
	move.l	d7,d6
	and.l	#OFFSET_MASK,d7
	move.l	d7,player.x.offset	this needed ?
	lsr.l	d5,d6
	move.w	d6,player.x.square

	move.l	player.z(pc),d7
	move.l	d7,d6
	and.l	#OFFSET_MASK,d7
	move.l	d7,player.z.offset	this needed ?
	lsr.l	d5,d6
	move.w	d6,player.z.square
	rts


player.direction			* angle = (0-1023)*2
	move.w	player.y.angle(pc),d7
	add.w	#256,d7			plus 45 degrees
	moveq	#9,d6
	lsr.w	d6,d7			find 90 degree section
	and.w	#3,d7
	move.l	.dir.table(pc,d7.w*4),a0
	jmp	(a0)


.dir.table
	dc.l	.north,.east,.south,.west


.north	move.w	level.width(pc),d7
	addq.w	#1,d7
	move.w	d7,row.size

	move.w	#-1,offset.left
	move.w	#1,offset.right
	move.w	d7,offset.forward

	move.w	player.x.square(pc),d6
	move.w	d6,count.left
	sub.w	d6,d7
	move.w	d7,count.right
	move.w	level.depth(pc),d6
	sub.w	player.z.square(pc),d6
	move.w	d6,count.forward

	move.w	#0,min.x.square
	move.w	level.width(pc),max.x.square

	move.w	player.z.square(pc),min.z.square
	move.w	level.depth(pc),max.z.square
	rts


.east	move.w	player.x.square(pc),min.x.square
	move.w	level.width(pc),max.x.square

	move.w	#0,min.z.square
	move.w	level.depth(pc),max.z.square
	rts


.south	move.w	#0,min.x.square
	move.w	level.width(pc),max.x.square

	move.w	#0,min.z.square
	addq.w	#1,player.z.square
	move.w	player.z.square(pc),max.z.square
	rts


.west	move.w	#0,min.x.square
	addq.w	#1,player.x.square
	move.w	player.x.square(pc),max.x.square
...
offset.left	dc.w	0
offset.right	dc.w	0
offset.forward	dc.w	0

count.left	dc.w	0
count.right	dc.w	0
count.forward	dc.w	0

min.x.square	dc.w	0		Only used by overhead view
max.x.square	dc.w	0
min.z.square	dc.w	0
max.z.square	dc.w	0
...
player.visibility
	move.w	row.size(pc),d7

	move.w	offset.left(pc),d6
	move.w	offset.right(pc),d5

	move.w	count.left(pc),d4
	move.w	count.right(pc),d3

	lea	level.coords(pc),a5	get array positions
	lea	level.coords.visible(pc),a4
	lea	screen.coords(pc),a3
	move.w	player.z.square(pc),d2
	mulu	d7,d2
	add.w	player.x.square(pc),d2	(need longword for very large maps)
	lea	(a5,d2.w*4),a5
	lea	(a4,d2.w),a4
	lea	(a3,d2.w*4),a3
	...
	lea	cosine(pc),a2
	move.w	(a2,d2.w),cos.y
	lea	sine(pc),a2
	move.w	(a2,d2.w),sin.y

* First, rotate all points in row behind player

.behind.left

* work out wall position relative to the player

	movem.l	(a5),d0-d1		wall offset in world
	sub.l	player.x.offset(pc),d0
	sub.l	player.z.offset(pc),d1
	swap	d0			X
	swap	d1			Z

	move.w	d0,d2
	move.w	d1,d3
	muls	sin.y(pc),d2		X(siny)
	muls	cos.y(pc),d1		Z(cosy)
	sub.l	d2,d1			rotated Z
	bgt.s	.in.front
...
.in.front
	muls	cos.y(pc),d0		X(cosy)
	muls	sin.y(pc),d3		Z(siny)
	add.l	d3,d0			rotated X

;	movem.l	d0-d1,(an)		save wall's transformed offsets

	cmp.l	d1,d0
	bgt.s	.behind			if object off right

	add.l	d1,d0
	blt.s	.behind			if object off left

;	cmp.l	#VISION.LIMIT,d1
;	bgt.s	.behind			if object too far away
...
screen.coords				* View from below
	ds.l	2*(MAX_LEVEL_WIDTH+1)*(MAX_LEVEL_DEPTH+1)


level.coords.visible	* View from below  * Only used by overhead view
	ds.b	(MAX_LEVEL_WIDTH+1)*(MAX_LEVEL_DEPTH+1)
	even
...
draw.overhead.view
	...
	lea	(SCREEN_WIDTH/8*OVERHEAD_Y)+(OVERHEAD_X/8)(a5),a5
	move.l	a5,overhead.screen

	bsr	print.player.pos
	bsr	draw.overhead.x.walls
	bsr	draw.overhead.z.walls
	bsr	clear.plane.2
	bsr	draw.overhead.player
	bsr	clear.plane.3
	bsr	draw.overhead.visible.x.walls
	bsr	draw.overhead.visible.z.walls
	rts


draw.overhead.x.walls
	lea	x.walls.map(pc),a5
	move.l	overhead.screen(pc),a4

	move.w	level.depth(pc),d7

.depth	move.l	a4,a3
	move.w	level.width(pc),d6
	subq.w	#1,d6

.width	cmp.b	#'-',(a5)+
	bne.s	.next

.wall	move.b	#$ff,(a3)		draw wall

.next	addq.w	#1,a3
	dbra	d6,.width

	lea	SCREEN_WIDTH/8*8(a4),a4
	dbra	d7,.depth
	rts


draw.overhead.z.walls
	lea	z.walls.map(pc),a5
	move.l	overhead.screen(pc),a4
	lea	SCREEN_WIDTH/8(a4),a4
...
draw.overhead.player
	move.l	level.z.max(pc),d7
	subq.l	#1,d7
	sub.l	player.z(pc),d7
	moveq	#OFFSET_SHIFT-3,d6
	lsr.l	d6,d7			y line number
	addq.w	#1,d7
	move.w	d7,player.scr.y

	move.l	player.x(pc),d7
	lsr.l	d6,d7			x pixel position
	move.w	d7,player.scr.x

* draw players direction line

	moveq	#12,d6
	move.w	player.y.angle(pc),d5
	bsr	direction.line

* draw players left sweep line

	moveq	#9,d6
	move.w	player.y.angle(pc),d5
	sub.w	#254,d5			minus 44 degrees
	and.w	#$7fe,d5
	bsr	direction.line

* draw players right sweep line

	moveq	#9,d6
	move.w	player.y.angle(pc),d5
	add.w	#256,d5			plus 45 degrees
	and.w	#$7fe,d5
	bsr	direction.line
	rts
...
* d6 = shift value for line length (smaller value gives longer line)
* d5 = line angle

	lea	cosine(pc),a1
	move.w	(a1,d5.w),d4		cosy
	asr.w	d6,d4

	lea	sine(pc),a1
	move.w	(a1,d5.w),d5		siny
	asr.w	d6,d5

	move.w	player.scr.x(pc),d0
	move.w	d0,d2
	add.w	d5,d2
	move.w	player.scr.y(pc),d1
	move.w	d1,d3
	sub.w	d4,d3
	add.w	#OVERHEAD_X,d0
	add.w	#OVERHEAD_X,d2
	...
	lea	SCREEN_WIDTH/8*SCREEN_HEIGHT(a1),a1	bitplane 2
	bra	draw.line.one.plane


;.plot	move.l	overhead.screen(pc),a4
;	lea	SCREEN_WIDTH/8*SCREEN_HEIGHT(a4),a4	bitplane 2
;
;	move.w	player.scr.y(pc),d7
;	lea	y.table(pc),a2
;	add.w	(a2,d7.w*2),a4		add y offset
;
;	move.w	player.scr.x(pc),d7
;	move.w	d7,d6
;	asr.w	#3,d6
;	add.w	d6,a4
;	not.w	d7
;	and.w	#7,d7
;	bset	d7,(a4)			plot player point
;	rts
...
draw.overhead.visible.x.walls
	move.l	overhead.screen(pc),a4
	lea	SCREEN_WIDTH/8*SCREEN_HEIGHT*2(a4),a4	bitplane 3

	move.w	min.x.square(pc),d4
	add.w	d4,a4
	move.w	max.x.square(pc),d5
	sub.w	d4,d5
	subq.w	#1,d5

	move.w	max.z.square(pc),d7
	move.w	level.depth(pc),d4
	sub.w	d7,d4
	mulu	#SCREEN_WIDTH/8*8,d4
	lea	(a4,d4.l),a4
	sub.w	min.z.square(pc),d7

.depth	move.l	a4,a3
	move.w	d5,d6

.width	move.b	#$7f,(a3)+		draw wall
	dbra	d6,.width
...
draw.overhead.visible.z.walls
	move.l	overhead.screen(pc),a4
	lea	SCREEN_WIDTH/8*(SCREEN_HEIGHT*2+1)(a4),a4	bitplane 3
...
.width	or.b	#$80,280(a3)		draw wall
	or.b	#$80,240(a3)
	or.b	#$80,200(a3)
	or.b	#$80,160(a3)
	or.b	#$80,120(a3)
...
player.x.square	dc.w	0
player.z.square	dc.w	0
...
clear.plane.3
	...
	lea	SCREEN_WIDTH/8*SCREEN_HEIGHT*2(a4),a4
	bra.s	clear.plane

clear.plane.2
	move.l	screen1(pc),a4
	lea	SCREEN_WIDTH/8*SCREEN_HEIGHT(a4),a4
...
fill.loop
	...
	asr.w	#2,d1			width of fill - 1, in words * 4
	beq.s	one.word.fill
