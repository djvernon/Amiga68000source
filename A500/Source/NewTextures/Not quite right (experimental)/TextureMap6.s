...
LOG_TEXTURE_SCALE	equ	6

LOG_FOCUS	equ	8	cannot be more than 8

* texture limit
TEXTURE_MAX	equ	TEXTURE_SIZE-1
...
* TOFFSET.w = offset into texture data (longwords)
...
READ_PIXEL	MACRO	; LAST PIXEL IN RUN (0/1), FIRST PIXEL IN BYTE (0/1)
	...
	ELSE		; last pixel in run (currently no difference)
...
STORE_SINGLE_BYTE	MACRO
	move.b	start.mask(pc),d0
	or.b	end.mask(pc),d0		combine start/end masks
	STORE_PARTIAL_BYTE
	sf.b	first.byte
	ENDM
...
standard.length.table
	dc.w	standard.length.offset0-last.length.offset0
	dc.w	standard.length.offset1-last.length.offset0
	dc.w	standard.length.offset2-last.length.offset0
	dc.w	standard.length.offset3-last.length.offset0
	dc.w	standard.length.offset4-last.length.offset0
	dc.w	standard.length.offset5-last.length.offset0
	dc.w	standard.length.offset6-last.length.offset0
	dc.w	standard.length.offset7-last.length.offset0
...
get.start.mask
	...
	add.w	d2,d2
	move.w	standard.length.table(pc,d2.w),sjo
...
tyend.shift1
tyend.ok1
* calculate initial offset into pixel data
	move.w	TYI,TOFFSET
	asl.w	#LOG_TEXTURE_SIZE,TOFFSET
	add.w	TXI,TOFFSET
	add.w	TOFFSET,TOFFSET
	add.w	TOFFSET,TOFFSET		longword offset into pixel data
	bra.s	do.scan.line
...
txend.ok2
	...
	asr.l	#LOG_FOCUS,TY
	add.l	VLINE(pc),TY
	asl.l	#LOG_TEXTURE_SCALE,TY
	divs	d0,TY		tyend
* limit tyend to texture size
	bpl.s	tyend.plus2
	moveq	#0,TY
*	bra.s	tyend.ok2
	bra.s	calc.distances
tyend.plus2
...
* TXSTART.w = txstart, TYSTART.w = tystart
* TX.w = txend, TY.w = tyend
* LENGTH always greater than zero
calc.distances
	move.w	TX,d0
	sub.w	TXSTART,d0	txend - txstart
	move.w	TX,TXSTART	store for next section of scan line

* calculate txstep start address
	lea	txsteps.zero(pc),a1
	asl.w	#LOG_LINEAR_SIZE+1,d0
	add.w	d0,a1

	move.w	TY,d0
	sub.w	TYSTART,d0	tyend - tystart
	move.w	TY,TYSTART	store for next section of scan line

* calculate tystep start address
	lea	tysteps.zero(pc),a6
	asl.w	#LOG_LINEAR_SIZE+1,d0
	add.w	d0,a6
...
.last	moveq	#0,DATA
	move.w	LENGTH,d0
	add.w	d0,d0
	move.w	.last.table(pc,d0.w),d0
	jmp	.last8(pc,d0.w)	do LENGTH pixels
...
* LENGTH 1-7
	...
	move.w	LENGTH,d0
	add.w	d0,d0
	move.w	.final.table(pc,d0.w),d0
	jmp	.final7(pc,d0.w)	do LENGTH pixels
...
.single	move.w	LENGTH,d0
	add.w	d0,d0
	move.w	.single.table(pc,d0.w),d0
	jmp	.singl7(pc,d0.w)	do LENGTH pixels
