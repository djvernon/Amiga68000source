...
;; display is now sequential - line routine will not currently work
;; re-insert original fill routine if need to view palette
...
loop
	...
	movem.w	base.x.angle,d0-d2	get x angle, y angle and z angle
...
;	move.l	#$2000000,z.offset
;	move.l	#$1000000,z.offset
	move.l	#Z_STEP,zstep
;	move.l	#0,zstep
...
;	bsr.s	planar.to.chunky
	bsr.s	planar.to.custom.chunky
	bsr.s	make.byte.to.word
	bsr	make.texture.steps
	rts


* make word.to.byte lookup table
make.byte.to.word
	lea	word.to.byte,a0
	moveq	#0,d0
.outer	moveq	#0,d1
...
planar.to.custom.chunky
	lea	bitmap,a0		* Interleaved
	lea	custom.chunky.data,a5
...
TEXTURE_SIZE	equ	64	width and height

MAX_DIST	equ	(TEXTURE_SIZE-1)

LOG_TEXTURE_SIZE	equ	6

COEFF_SHIFT	equ	2

LINEAR_SIZE	equ	32	routine is now hard-coded for this length
...
* texture limit
TEXTURE_MAX	equ	(1<<(LOG_TEXTURE_SIZE+LOG_TX_PREC))-1



make.texture.steps		* for range -MAX_DIST to MAX_DIST
	move.w	#-MAX_DIST,d7
	lea	txsteps(pc),a5
	lea	tysteps(pc),a6
...
.loop	move.w	d7,d6
	swap	d6
	clr.w	d6
	asr.l	#LOG_LINEAR_SIZE,d6	txinc

* make offsets from start
	moveq	#0,d5		txoffset
	move.w	#LINEAR_SIZE-1,d4
	lea	txoffsets(pc),a4
.next.offset
	add.l	d6,d5
	move.l	d5,d3
...
* make steps from previous texture position
	moveq	#0,d5
	lea	txoffsets(pc),a4
.next.step
	move.w	(a4)+,d4	txoffset
	move.w	d4,d3
	tst.w	d5
	beq.s	.store
...
.store	move.w	d4,d2		previous txoffset
	asl.w	#2,d3		longword offset into pixel data
	move.w	d3,(a5)+
	asl.w	#LOG_LINEAR_SIZE,d3
	move.w	d3,(a6)+

	addq.w	#1,d5
	cmp.w	#LINEAR_SIZE,d5
	blt.s	.next.step
...
txoffsets	ds.w	LINEAR_SIZE

txsteps	ds.w	MAX_DIST*LINEAR_SIZE		-MAX_DIST to -1
txsteps.zero
	ds.w	LINEAR_SIZE			0
	ds.w	MAX_DIST*LINEAR_SIZE		1 to MAX_DIST

tysteps	ds.w	MAX_DIST*LINEAR_SIZE		-MAX_DIST to -1
tysteps.zero
	ds.w	LINEAR_SIZE			0
	ds.w	MAX_DIST*LINEAR_SIZE		1 to MAX_DIST
...
* TX.l = txinc, TY.w = tyinc
* d4.l = txstart, d5.w = tystart
* d2-d4 are now also four times bigger (longword offset into pixel data)
* LOG_TX_PREC 16 for tx
* LOG_TX_PREC = LOG_TEXTURE_SIZE for ty
* d1 free
; LAST PIXEL IN RUN flag currently not required
READ_PIXEL	MACRO	; LAST PIXEL IN RUN (0/1), FIRST PIXEL IN BYTE (0/1)
	IFNC	'\1','1'
;	move.w	d5,d1
;	and.w	TYMASK,d1
;	swap	d4
...
;	and.w	TXMASK,d1
	...
	IFC	'\2','1'
	move.l	(a3,TOFFSET.w),DATA
	ELSE
	add.l	DATA,DATA
	or.l	(a3,TOFFSET.w),DATA
	ENDC
...
; TEMPORARILY FREE UP a6
* store 8 pixels to four bitplanes
STORE_WHOLE_BYTE	MACRO
	move.b	DATA,(a0)+
	move.b	(a2,DATA.w),(a4)+
	swap	DATA
	move.b	DATA,(a5)+
;	move.b	(a2,DATA.w),(a6)+
	move.b	(a2,DATA.w),8000-1(a5)
	ENDM

STORE_PARTIAL_BYTE	MACRO
	and.b	d1,(a0)
	and.b	d1,(a4)
	and.b	d1,(a5)
;	and.b	d1,(a6)
...
STORE_SINGLE_BYTE	MACRO
	move.b	start.mask(pc),d1
	or.b	end.mask(pc),d1		combine start/end masks
	STORE_PARTIAL_BYTE
	sf.b	first.byte
	ENDM

STORE_FIRST_BYTE	MACRO
	move.b	start.mask(pc),d1
	STORE_PARTIAL_BYTE
	sf.b	first.byte
	ENDM

STORE_LAST_BYTE	MACRO
	move.b	end.mask(pc),d1
	STORE_PARTIAL_BYTE
	ENDM
...
NEXT_LINE	MACRO
	...
	move.l	current.screen.line(pc),a0
	lea	SCREEN_WIDTH/8(a0),a0	next line

	move.l	current.fill.coords(pc),a1
	move.w	(a1)+,XSTART		next x-start
	bpl	fill.loop
	bra	end.texture.fill
	ENDM


; other optimisations:
; replace divides with lookup and multiply
; use X1 less - instead use LENGTH to test for end of line etc.
;
; can probably remove swaps from last.length routines as d0 etc. now free
...
.wait	btst	#6,$dff000+dmaconr
	bne.s	.wait

	move.w	(a1)+,XSTART		first x-start
	bpl.s	fill.loop
...
start.masks
	...
	dc.b	$00,$7f,$3f,$1f,$0f,$07,$03,$01

standard.length.table
	dc.l	standard.length.offset0,standard.length.offset1
	dc.l	standard.length.offset2,standard.length.offset3
	dc.l	standard.length.offset4,standard.length.offset5
	dc.l	standard.length.offset6,standard.length.offset7

last.length.table
	dc.w	last.length.offset0-last.length.offset0
	dc.w	last.length.offset1-last.length.offset0
	dc.w	last.length.offset2-last.length.offset0
	dc.w	last.length.offset3-last.length.offset0
	dc.w	last.length.offset4-last.length.offset0
	dc.w	last.length.offset5-last.length.offset0
	dc.w	last.length.offset6-last.length.offset0
	dc.w	last.length.offset7-last.length.offset0


* loop, drawing each scan-line

fill.loop
	move.l	a0,current.screen.line

	move.w	(a1)+,XEND		next x-end
	move.l	a1,current.fill.coords

	cmp.w	XSTART,XEND
	ble	next.line		if x-end <= x-start
* x-start and x-end are already limited to screen boundaries

	moveq	#7,d2
	and.w	XSTART,d2
	beq.s	get.start.mask		first byte checking not needed
	st.b	first.byte

get.start.mask
	move.b	start.masks(pc,d2.w),start.mask
	add.w	d2,d2
	move.w	last.length.table(pc,d2.w),last.jump.offset
	add.w	d2,d2
	move.l	standard.length.table(pc,d2.w),a7

	moveq	#7,d2
	and.w	XEND,d2
	move.b	end.masks(pc,d2.w),end.mask

	moveq	#0,DATA

	move.w	XSTART,d2
	asr.w	#3,d2
	add.w	d2,a0		start address of fill
	lea	8000(a0),a4
	lea	8000(a4),a5
;	lea	8000(a5),a6
...
TXI	equr	d4	* register used to create initial tx
TYI	equr	d5	* register used to create initial ty
...
*		}
;	move.w	X0(pc),TXI	not needed when TXI == TXSTART
	move.w	TXI,TYI
	muls	ZI(pc),TXI
	asr.l	#LOG_FOCUS,TXI
	add.l	ZLINE(pc),TXI
	move.w	TXI,d1		zpixel
	...
	asr.l	#LOG_FOCUS,TXI
	add.l	ULINE(pc),TXI
	asr.l	#(16-LOG_TEXTURE_SCALE),TXI
	swap	TXI		gives left shift of LOG_TEXTURE_SCALE
	clr.w	TXI
	divs	d1,TXI		txend
* limit txend to texture size
	...
	bra.s	txend.ok1
txend.plus1
	cmp.w	#TEXTURE_MAX,TXI
	ble.s	txend.shift1
	move.l	#TEXTURE_MAX<<(16-6+2),TXI	(incorporates shift below)
	bra.s	txend.ok1
txend.shift1
	swap	TXI		increase tx precision to 16
	clr.w	TXI
	asr.l	#6-2,TXI		longword offset into pixel data
txend.ok1

	muls	VI(pc),TYI
	asr.l	#LOG_FOCUS,TYI
	add.l	VLINE(pc),TYI
	asr.l	#(16-LOG_TEXTURE_SCALE),TYI
	swap	TYI		gives left shift of LOG_TEXTURE_SCALE
	clr.w	TYI
	divs	d1,TYI		tyend
* limit tyend to texture size
	...
	moveq	#0,TYI
*	bra.s	tyend.ok1
	bra.s	do.scan.line.first
tyend.plus1
	cmp.w	#TEXTURE_MAX,TYI
	ble.s	tyend.shift1
	move.w	#TEXTURE_MAX,TYI
tyend.shift1
	add.w	TYI,TYI
	add.w	TYI,TYI		longword offset into pixel data
tyend.ok1
	bra.s	do.scan.line.first

TX	equr	d2	* register used to create tx
TY	equr	d3	* register used to create ty
TXSTART	equr	d4
TYSTART	equr	d5
;TXEND saved to txend
;TYEND saved to tyend
...
do.scan.line
* TX.l = txend, TY.w = tyend
	move.l	txend(pc),TXSTART	txstart = txend
	move.w	tyend(pc),TYSTART	tystart = tyend

do.scan.line.first
	* do first section of scan line
	* txstart and tystart are already in correct registers

; need to calculate correct texture start offset
; needs optimising and also only do at start of line
	move.w	TYSTART,TOFFSET
	and.w	#$ff00,TOFFSET
	swap	TXSTART
	add.w	TXSTART,TOFFSET
	swap	TXSTART
	and.w	#$fffc,TOFFSET
;	moveq	#0,TOFFSET

* calculate length - unlike PC version, doesn't draw last pixel
...
;	move.w	X0(pc),TY
	...
	asr.l	#LOG_FOCUS,TX
	add.l	ULINE(pc),TX
	asr.l	#(16-LOG_TEXTURE_SCALE),TX
	swap	TX		gives left shift of LOG_TEXTURE_SCALE
	clr.w	TX
	divs	d0,TX		txend
* limit txend to texture size
	bpl.s	txend.plus2
	...
	bra.s	txend.ok2
txend.plus2
	cmp.w	#TEXTURE_MAX,TX
	ble.s	txend.shift2
	move.l	#TEXTURE_MAX<<(16-6+2),TX	(incorporates shift below)
	bra.s	txend.ok2
txend.shift2
	swap	TX		increase tx precision to 16
	clr.w	TX
	asr.l	#6-2,TX		longword offset into pixel data
txend.ok2

	muls	VI(pc),TY
	asr.l	#LOG_FOCUS,TY
	add.l	VLINE(pc),TY
	asr.l	#(16-LOG_TEXTURE_SCALE),TY
	swap	TY		gives left shift of LOG_TEXTURE_SCALE
	clr.w	TY
	divs	d0,TY		tyend
* limit tyend to texture size
	bpl.s	tyend.plus2
...
tyend.plus2
	cmp.w	#TEXTURE_MAX,TY
	ble.s	tyend.shift2
	move.w	#TEXTURE_MAX,TY
tyend.shift2
	add.w	TY,TY
	add.w	TY,TY		longword offset into pixel data
tyend.ok2
	bra.s	calc.increments
...
* TXSTART.l = txstart, TYSTART.w = tystart
* TX.l = txend, TY.w = tyend
* LENGTH always greater than zero
calc.increments
...
*		}
	move.l	TX,txend
	sub.l	TXSTART,TX	txend - txstart

	move.w	TY,tyend	save
	sub.w	TYSTART,TY	tyend - tystart

; need to use two distances above to get correct txsteps addresses
; optimise this - remove shifts ans incorporate in tx/ty calculations
	lea	txsteps.zero(pc),a1	x
	swap	TX
	asl.w	#LOG_LINEAR_SIZE+1-2,TX
	add.w	TX,a1

	lea	tysteps.zero(pc),a6	y
	lea	64(a6),a6
;	asr.w	#8,TY
;	asl.w	#LOG_LINEAR_SIZE+1,TY
;	add.w	TY,a6

	cmp.w	#LINEAR_SIZE,LENGTH
	bne.s	last.linear.run
...
last.linear.run
	move.w	last.jump.offset(pc),d0
	jmp	last.length.offset0(pc,d0.w)


* last length (1 to 31 pixels) routines for start offsets 0 to 7
...
* no need to check first.byte when offset 0
...
.last	moveq	#0,DATA
	...
	swap	LENGTH
	add.w	d0,d0
	move.w	.last.table(pc,d0.w),d1
	jmp	.last8(pc,d1.w)	do LENGTH pixels

.last.table
	dc.w	0,.last1-.last8,.last2-.last8,.last3-.last8,.last4-.last8
	dc.w	.last5-.last8,.last6-.last8,.last7-.last8,.last8-.last8

.last8	READ_PIXEL	0,1
.last7	READ_PIXEL	0,0
.last6	READ_PIXEL	0,0
.last5	READ_PIXEL	0,0
.last4	READ_PIXEL	0,0
.last3	READ_PIXEL	0,0
.last2	READ_PIXEL	0,0
.last1	READ_PIXEL	1,0
	swap	LENGTH
	moveq	#8,d0
	sub.w	LENGTH,d0
	asl.l	d0,DATA
...
* LENGTH 1-7
	...
	swap	LENGTH
	add.w	d0,d0
	move.w	.final.table(pc,d0.w),d1
	jmp	.final7(pc,d1.w)	do LENGTH pixels

.final.table
	dc.w	0,.final1-.final7,.final2-.final7,.final3-.final7
	dc.w	.final4-.final7,.final5-.final7,.final6-.final7,.final7-.final7

.final7	READ_PIXEL	0,1
.final6	READ_PIXEL	0,0
.final5	READ_PIXEL	0,0
.final4	READ_PIXEL	0,0
.final3	READ_PIXEL	0,0
.final2	READ_PIXEL	0,0
.final1	READ_PIXEL	1,0
	swap	LENGTH
	moveq	#8,d0
	sub.w	LENGTH,d0
	asl.l	d0,DATA
...
* line's first byte
.first	cmp.w	d0,LENGTH
	bgt	.not.single

* LENGTH 1-7
.single	move.w	LENGTH,d0
	swap	LENGTH
	add.w	d0,d0
	move.w	.single.table(pc,d0.w),d1
	jmp	.singl7(pc,d1.w)	do LENGTH pixels

.single.table
	dc.w	0,.singl1-.singl7,.singl2-.singl7,.singl3-.singl7
	dc.w	.singl4-.singl7,.singl5-.singl7,.singl6-.singl7,.singl7-.singl7

.singl7	READ_PIXEL	0,1
.singl6	READ_PIXEL	0,0
.singl5	READ_PIXEL	0,0
.singl4	READ_PIXEL	0,0
.singl3	READ_PIXEL	0,0
.singl2	READ_PIXEL	0,0
.singl1	READ_PIXEL	1,0
	swap	LENGTH
	moveq	#8-OFFSET1,d0
	sub.w	LENGTH,d0
	asl.l	d0,DATA
...
.not.single
	subq.w	#8-OFFSET1,LENGTH
	swap	LENGTH

	READ_PIXEL	0,1		do (8-offset) pixels
	REPT	8-OFFSET1-1
	READ_PIXEL	0,0
	ENDR
...
* not line's first byte
.not.first
	cmp.w	d0,LENGTH
	bgt	.not.last

* LENGTH 1-8
.last	move.w	LENGTH,d0
	swap	LENGTH
	add.w	d0,d0
	move.w	.last.table(pc,d0.w),d1
	jmp	.last8(pc,d1.w)		do LENGTH pixels
...
.not.last
	...
	REPT	8-OFFSET1		do (8-offset) pixels
	READ_PIXEL	0,0
	ENDR
...
.not.last
	...
	REPT	8-OFFSET2		do (8-offset) pixels
	READ_PIXEL	0,0
	ENDR
...
.not.last
	...
	REPT	8-OFFSET3		do (8-offset) pixels
	READ_PIXEL	0,0
	ENDR
...
.not.last
	...
	REPT	8-OFFSET4		do (8-offset) pixels
	READ_PIXEL	0,0
	ENDR
...
.not.last
	...
	REPT	8-OFFSET5		do (8-offset) pixels
	READ_PIXEL	0,0
	ENDR
...
.not.last
	...
	REPT	8-OFFSET6		do (8-offset) pixels
	READ_PIXEL	0,0
	ENDR
...
.not.last
	...
	REPT	8-OFFSET7		do (8-offset) pixels
	READ_PIXEL	0,0
	ENDR
...
* standard length (32 pixels) routines for start offsets 0 to 7

standard.length.offset0
	REPT	3
	READ_PIXEL	0,1
	READ_PIXEL	0,0
	READ_PIXEL	0,0
...
* line first byte flag
first.byte	dc.b	0
	even

last.jump.offset	dc.w	0

current.screen.line	dc.l	0
current.fill.coords	dc.l	0
...
;"""""""""""""""""
	...
	ds.b	32768
word.to.byte		* lookup is done using sign extended word
	ds.b	32768
...
custom.chunky.data	ds.l	SOURCE_WIDTH*SOURCE_HEIGHT
* blank line after
		ds.l	SOURCE_WIDTH
