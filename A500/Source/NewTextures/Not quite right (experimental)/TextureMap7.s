...
get.start.mask
	...
	add.w	d2,d2
	move.w	standard.length.table(pc,d2.w),JUMP_OFFSET
...
LENGTH	equr	d1	* same as JUMP_OFFSET
TX	equr	d2	(TXEND)
TY	equr	d3	(TYEND)
TXSTART	equr	d4
TYSTART	equr	d5
...
;	move.w	X1(pc),TY
;	add.w	TY,d0
	add.w	X1(pc),d0	use standard length for calculation

	move.w	d0,LENGTH
	swap	LENGTH
	move.w	last.jump.offset(pc),JUMP_OFFSET
...
* calculate tystep start address
	lea	tysteps.zero(pc),a7
	asl.w	#LOG_LINEAR_SIZE+1,d0
	add.w	d0,a7

begin.linear.run
	jmp	last.length.offset0(pc,JUMP_OFFSET.w)
