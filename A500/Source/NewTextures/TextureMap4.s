...
* texture limits
TXMAX	equ	(1<<(LOG_TEXTURE_WIDTH+LOG_TX_PREC))-1
TYMAX	equ	(1<<(LOG_TEXTURE_HEIGHT+LOG_TX_PREC))-1
...
* LOG_TX_PREC = LOG_TEXTURE_WIDTH for ty
* d1 free
...
* calculate co-efficients for texture mapping
	...
	move.w	#%111111000000<<2,TYMASK	long offset into pixel data
...
txend.plus1
	cmp.w	#TXMAX,TXI
	ble.s	txend.shift1
	move.l	#TXMAX<<(16-6+2),TXI	(incorporates shift below)
	bra.s	txend.ok1
txend.shift1
...
txend.plus2
	cmp.w	#TXMAX,TX
	ble.s	txend.shift2
	move.l	#TXMAX<<(16-6+2),TX	(incorporates shift below)
	bra.s	txend.ok2
txend.shift2
...
*		}
	...
	beq.s	standard.linear.run
	cmp.w	#1,LENGTH
	beq.s	last.linear.run
	asr.l	#8,TX		prevent txinc overflow
	divs	LENGTH,TX
	ext.l	TX
	asl.l	#8,TX
	...
	ext.l	TY
	divs	LENGTH,TY
	bra.s	last.linear.run

standard.linear.run
	asr.l	#LOG_LINEAR_SIZE,TX	txinc
	asr.w	#LOG_LINEAR_SIZE,TY	tyinc
	moveq	#%111111<<2,TXMASK	long offset into pixel data
	jmp	(a7)
