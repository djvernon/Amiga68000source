...
set.x.y.z.angles
	tst.b	auto.rotate
	bne.s	auto.rotate.x.y.z.angles
...
LINEAR_SIZE	equ	16
LOG_LINEAR_SIZE	equ	4
LOG_TX_PREC	equ	10		precision
...
*		}
	...
	asr.l	#LOG_FOCUS,d0
	add.l	ZLINE(pc),d0
;	swap	d0		zpixel - gives Log_Texture_Scale (+ LOG_TX_PRECISION) of 16
	move.w	d0,d2
...
;; nicer to use d1 below - do by assigning tx/tystart before length above
	muls	VI(pc),d6
	asr.l	#LOG_FOCUS,d6
	add.l	VLINE(pc),d6
	swap	d6
...
*		}
	tst.w	LENGTH		;; is this ever zero ?
	bne.s	non.zero.length
	moveq	#0,d2
	moveq	#0,d3
