...
; other optimisations:
; register usage
; one detailed before txend.shift
; replace divides with lookup and multiply
; use X1 less - instead use LENGTH to test for end of line etc.
; use immediate data for txmax, tymax
; READ_PIXEL:
;	remove the two add.w dn,dn before longword read
;	try to remove one swap d4
; give standard.length.offsetN next.line code instead of bra next.line
; get rid of move.w d1,d3, by using d1 for tyinc
; use jump tables for 'do LENGTH pixels' and 'do whole bytes' of last run
...
txend.plus1
	cmp.w	txmax(pc),d0
	ble.s	txend.shift1
	move.w	txmax(pc),d0	could convert to move.l then bra.s txend.ok1
txend.shift1
	swap	d0		increase tx precision to 16
	clr.w	d0
	asr.l	#6,d0
txend.ok1
...
* d0.l = txend, d1.w = tyend
* loop, drawing each pixel on current scan-line
do.scan.line
	move.l	d0,d4	txstart = txend
...
* d4.l = txstart, d5.w = tystart
* d0.l = txend, d1.w = tyend
* d3.w = length (always greater than zero)
calc.increments
...
*		}
	move.l	d0,d2
	sub.l	d4,d2		txend - txstart

	move.w	d1,tyend	save
	sub.w	d5,d1		tyend - tystart

	cmp.w	#LINEAR_SIZE,LENGTH
	beq.s	standard.linear.run
	cmp.w	#1,LENGTH
	beq.s	last.linear.run
	asr.l	#6,d2		prevent overflow
	divs	LENGTH,d2
	ext.l	d2
	asl.l	#6,d2
	ext.l	d1
...
standard.linear.run
	asr.l	#LOG_LINEAR_SIZE,d2	txinc
	move.w	d1,d3
...
last.length.offsetN
	moveq	#8,d1
	sub.w	temp.offset(pc),d1
	move.w	d1,eight.minus.offset
...
.single	move.w	d0,d1
	...
	move.w	d1,d0
.single.loop
	READ_PIXEL	0,0			do LENGTH pixels
	dbra	d0,.single.loop
	swap	d0
	move.b	end.mask(pc),d1
	or.b	d1,start.mask	combine start/end masks
	move.w	eight.minus.offset(pc),d1
	sub.w	d0,d1
	asl.l	d1,DATA
	STORE_FIRST_BYTE		don't need to sf.b first.byte here
	bra	next.line
...
.not.single
	...
	beq	.next
	move.w	eight.minus.offset(pc),d1
	sub.w	d1,d0			length -= (8-offset)
	subq.w	#1,d1
	swap	d0
	move.w	d1,d0
.not.single.loop
	READ_PIXEL	0,0			do (8-offset) pixels
	dbra	d0,.not.single.loop
	swap	d0
	STORE_FIRST_BYTE		don't need to sf.b first.byte here
	bra	.next

* not line's first byte (or offset 0)
.not.first
	cmp.w	d1,d0
	bgt.s	.not.last

.last	tst.w	temp.offset
	bne.s	.data.ok
	moveq	#0,DATA
...
* do last byte
	...
	move.w	d1,d0
.last.loop2
; can't take advantage of final READ_PIXEL
	READ_PIXEL	0,0			do LENGTH pixels
	dbra	d0,.last.loop2
	swap	d0
