...
start	move.l	4.w,a6
	...
	lea	4*40*6+2(a1),a4
	move.l	a4,current.screen

	lea	screen.data(pc),a0
	move.w	#4*40*200/4-1,d1
screen.copy
	move.l	(a0)+,d0
	move.l	d0,(a1)+
	move.l	d0,(a2)+
...
loop	movem.w	base.x.angle(pc),d0-d2	get x angle, y angle and z angle
	...
	move.w	mouse.x(pc),d0
	bsr	make.decimal
	lea	decimal.text(pc),a0
	moveq	#15,d0			x
	moveq	#64,d1			y
	bsr	print
	move.w	mouse.y(pc),d0
	bsr	make.decimal
	lea	decimal.text(pc),a0
	moveq	#15,d0			x
	moveq	#72,d1			y
...
clear	btst	#6,dmaconr(a6)
	...
	move.l	current.screen(pc),bltdpth(a6)
	move.w	#140*4*64+18,bltsize(a6)
	rts
...
check.icons
	tst.b	left.mouse.button
	beq.s	check.icons.right.button

	move.w	left.mouse.button.x(pc),d0
	move.w	left.mouse.button.y(pc),d1
	sf	left.mouse.button

	cmp.w	#152,d1
	blt.s	end.check.icons
...
check.icon1d
	...
	blt.s	end.check.icons
	cmp.w	#40,d0
	bgt.s	end.check.icons

	add.w	#40,z.offset
	lea	icon1.textd(pc),a0
	moveq	#4,d0
	moveq	#96,d1
	bra	print
...
check.icon2a
	cmp.w	#175,d1
	bgt.s	icon2b.selected

	subq.w	#4,y.offset
	lea	icon2.texta(pc),a0
	moveq	#4,d0
	moveq	#96,d1
	bra	print
...
check.icon3a
	...
	subq.w	#8,turret.y.angle
	lea	icon3.texta(pc),a0
	moveq	#4,d0
	moveq	#96,d1
	bra	print
...
check.icon3b
	...
	addq.w	#8,turret.y.angle
	lea	icon3.textb(pc),a0
	moveq	#4,d0
	moveq	#96,d1
	bra	print
...
check.icon4a
	cmp.w	#175,d1
	bgt.s	icon4b.selected

	lea	icon4.texta(pc),a0
	moveq	#4,d0
	moveq	#96,d1
	bra	print

icon4b.selected
	lea	icon4.textb(pc),a0
	moveq	#4,d0
	moveq	#96,d1
	bra	print
...
icon1.texta	dc.b	'World forwards',0
icon1.textb	dc.b	'World left',0
icon1.textc	dc.b	'World right',0
icon1.textd	dc.b	'World backwards',0

icon2.texta	dc.b	'Turret up',0
icon2.textb	dc.b	'Turret down',0

icon3.texta	dc.b	'Turret left',0
icon3.textb	dc.b	'Turret right',0

icon4.texta	dc.b	'More power',0
icon4.textb	dc.b	'Less power',0
icon4.textc	dc.b	'Shell fired',0

icon5.text	dc.b	'Tank movement',0

icon6.text	dc.b	'Icon6 region',0
	even
...
next.column
	...
	add.w	#36,d2			next start x
	dbra	d3,next.column

	add.w	#17,d0			next start y
	dbra	d1,next.row
	rts
...
fill.box
	...
	move.w	d2,d3
	add.w	#31,d3			32 pixels wide
	moveq	#15-1,d1		15 pixels tall
...
update.screens
	...
	move.l	screen1(pc),screen3
	move.l	a0,screen1
	lea	4*40*6+2(a0),a0
	move.l	a0,current.screen
...
make.copper.lists
	move.l	#mouse.pointer,d0
	move.w	d0,copper.list1+38
	move.w	d0,copper.list2+38
	move.w	d0,copper.list3+38
	swap	d0
	move.w	d0,copper.list1+34
	move.w	d0,copper.list2+34
	move.w	d0,copper.list3+34
...
; inner track sides
	...
	dc.w	2+8*2
	dc.w	8
	dc.w	P9,P16,P15,P14,P13,P12,P11,P10

	dc.w	polygon.orientation.offset
	dc.w	P17,P20,P23
	dc.w	2+8*2
	dc.w	8
	dc.w	P17,P18,P19,P20,P21,P22,P23,P24
...
; two inner track covers
	...
	dc.w	2+4*2
	dc.w	4
	dc.w	P34,P35,P49,P42

	dc.w	polygon.orientation.offset
	dc.w	P37,P43,P50
	dc.w	2+4*2
	dc.w	4
	dc.w	P37,P43,P50,P36
...
; right track surfaces
	...
	dc.w	2+4*2
	dc.w	4
	dc.w	P19,P27,P28,P20

	dc.w	polygon.orientation.offset
	dc.w	P21,P29,P30
	dc.w	2+4*2
	dc.w	4
	dc.w	P21,P29,P30,P22
	...
	dc.w	2+4*2
	dc.w	4
	dc.w	P18,P26,P27,P19

	dc.w	polygon.orientation.offset
	dc.w	P22,P30,P31
	dc.w	2+4*2
	dc.w	4
	dc.w	P22,P30,P31,P23
	...
	dc.w	2+4*2
	dc.w	4
	dc.w	P17,P25,P26,P18

	dc.w	polygon.orientation.offset
	dc.w	P23,P31,P32
	dc.w	2+4*2
	dc.w	4
	dc.w	P23,P31,P32,P24
...
; outer track sides
	...
	dc.w	2+8*2
	dc.w	8
	dc.w	P1,P2,P3,P4,P5,P6,P7,P8

	dc.w	polygon.orientation.offset
	dc.w	P25,P31,P28
	dc.w	2+8*2
	dc.w	8
	dc.w	P25,P32,P31,P30,P29,P28,P27,P26
...
; two top track covers
	...
	dc.w	2+4*2
	dc.w	4
	dc.w	P47,P48,P54,P46

	dc.w	polygon.orientation.offset
	dc.w	P51,P52,P45
	dc.w	2+4*2
	dc.w	4
	dc.w	P51,P52,P45,P53
...
; two body sides
	...
	dc.w	2+4*2
	dc.w	4
	dc.w	P48,P55,P58,P54

	dc.w	polygon.orientation.offset
	dc.w	P51,P53,P57
	dc.w	2+4*2
	dc.w	4
	dc.w	P51,P53,P57,P56
...
; two outer track covers
	...
	dc.w	2+5*2
	dc.w	5
	dc.w	P33,P41,P47,P46,P40

	dc.w	polygon.orientation.offset
	dc.w	P52,P44,P38
	dc.w	2+5*2
	dc.w	5
	dc.w	P38,P39,P45,P52,P44
...
; two upper front track covers
	...
	dc.w	2+4*2
	dc.w	4
	dc.w	P41,P42,P49,P47

	dc.w	polygon.orientation.offset
	dc.w	P43,P44,P52
	dc.w	2+4*2
	dc.w	4
	dc.w	P43,P44,P52,P50
...
; two lower front track covers
	...
	dc.w	2+4*2
	dc.w	4
	dc.w	P33,P34,P42,P41

	dc.w	polygon.orientation.offset
	dc.w	P37,P38,P44
	dc.w	2+4*2
	dc.w	4
	dc.w	P37,P38,P44,P43
...
; gun
	...
	dc.w	2+4*2
	dc.w	4
	dc.w	P17,P20,P19,P18

	dc.w	polygon.orientation.offset
	dc.w	P21,P22,P23
	dc.w	2+4*2
	dc.w	4
	dc.w	P21,P22,P23,P24
	...
	dc.w	2+4*2
	dc.w	4
	dc.w	P17,P21,P24,P20

	dc.w	polygon.orientation.offset
	dc.w	P18,P19,P23
	dc.w	2+4*2
	dc.w	4
	dc.w	P18,P19,P23,P22
...
screen.data
	incbin	graphics_data/Game_Screen.bin
