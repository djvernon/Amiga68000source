	section	3D_KingTiger,code_c
	opt	o+,o3-


;	Made from Disks/3D_example.s on 29/01/2020
...
; 29/01/2020 NB now only reading base angles and z offset once per loop, to keep all objects (tank base and top) locked togther
; This is required because the angles are updated more frequently than this loop, by level 3 interrupt
loop	movem.w	base.x.angle(pc),d0-d2	get x angle, y angle and z angle
	movem.w	d0-d2,-(sp)		; 29/01/2020 save for later when drawing king.tiger.top
	bsr	calc.sin.cos.values

; 29/01/2020 copy z offset once
	move.l	base.z.offset(pc),z.offset

	lea	king.tiger.base(pc),a0
	bsr	draw.3d.object

; 29/01/2020 NB fetching angles again below had unwanted side effect of turret not being locked to base (appears to wobble etc.)
;	movem.w	base.x.angle(pc),d0-d2	get x angle, y angle and z angle
	movem.w	(sp)+,d0-d2		get x angle, y angle and z angle
...
right.pressed
	...
	ext.w	d1
	add.w	d1,base.z.offset	update z distance
