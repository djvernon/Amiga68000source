...
fill	st	(a1)			end-of-fill marker
	st	4(a1)			second end-of-fill marker for stipple
	move.l	screen.mem(pc),a0
	lea	fill.coords(pc),a1
	move.w	(a1)+,d2		get y-start
	add.w	d2,d2			word offset
	lea	y.table(pc),a2
	add.w	(a2,d2.w),a0		add y offset
...
stipple.fill
	move.l	a1,d3			save fill co-ords pointer for
	addq.l	#2,d3			second stipple-fill loop
	move.l	a0,-(sp)		also save screen pointer

* Save pointer to code that will make opposite masks for lines not filled
* during the first loop and will do the stipple loop a second time.

	pea	stipple.other.lines

	sub.w	#16*4,d1
	move.l	stipple.colour.table(pc,d1.w),a2
	jmp	(a2)



stipple.colour.table
	dc.l	stipple.colour1,stipple.colour2,stipple.colour3
	dc.l	stipple.colour4,stipple.colour5,stipple.colour6
	dc.l	stipple.colour7,stipple.colour8,stipple.colour9
	dc.l	stipple.colour10,stipple.colour11,stipple.colour12
	dc.l	stipple.colour13,stipple.colour14,stipple.colour15
...
stipple.colour1
	...
	moveq	#0,d6
	moveq	#0,d7
	lea	stipple.colour1.ins(pc),a2
	bra	stipple.copy.ins
...
stipple.colour10
	...
	moveq	#0,d6
	move.l	#$aaaaaaaa,d7
	lea	stipple.colour10.ins(pc),a2
	bra	stipple.copy.ins
...
stipple.colour11
	...
	moveq	#0,d6
	move.l	#$aaaaaaaa,d7
	lea	stipple.colour11.ins(pc),a2
	bra.s	stipple.copy.ins
...
stipple.colour12
	...
	move.l	#$aaaaaaaa,d6
	move.l	#$aaaaaaaa,d7
	lea	stipple.colour12.ins(pc),a2
	bra.s	stipple.copy.ins
...
stipple.colour13
	...
	move.l	#$aaaaaaaa,d6
	move.l	#$aaaaaaaa,d7
	lea	stipple.colour13.ins(pc),a2
	bra.s	stipple.copy.ins
...
stipple.colour14
	...
	move.l	#$aaaaaaaa,d6
	move.l	#$aaaaaaaa,d7
	lea	stipple.colour14.ins(pc),a2
	bra.s	stipple.copy.ins
...
stipple.colour15
	...
	move.l	#$aaaaaaaa,d6
	move.l	#$aaaaaaaa,d7
	lea	stipple.colour15.ins(pc),a2
...
even.scan.line
	cmp.w	old.stipple.colour(pc),d1
	beq.s	stipple.loop		if colour has not changed
	move.w	d1,old.stipple.colour

	lea	set.first.words2(pc),a3
	move.l	(a2)+,(a3)+
	move.l	(a2)+,(a3)+
	move.l	(a2)+,(a3)+
	move.l	(a2)+,(a3)+
...
stipple.loop
	move.w	(a1)+,d1		next x-end
	addq.l	#4,a1			skip next co-ordinate pair
	sub.w	d0,d1
	...
	add.w	d2,d2
	move.w	start.masks2(pc,d2.w),d0	get negative start mask
	...
	add.w	d2,d2
	move.w	end.masks2(pc,d2.w),d2	get negative end mask

	lsr.w	#2,d1			width of fill - 1, in words * 4
	beq.s	one.word.fill2
...
set.first.words2
	...
	and.w	d0,(a4)+
	and.w	d0,(a5)+
	not.w	d0			make positive start mask
	and.w	d4,d0
	or.w	d0,(a2)+
	nop
	nop
	...
	move.l	stipple.width.table-4(pc,d1.w),a6
	jmp	(a6)

one.word.fill2
	or.w	d0,d2			combine start and end masks
...
stipple.words2
	...
	and.w	d2,(a4)
	and.w	d2,(a5)
	not.w	d2			make positive end mask
	and.w	d4,d2
	or.w	d2,(a2)
	nop
	nop
...
next.line2
	lea	320(a0),a0		to next line but one
	move.w	(a1)+,d0		next x-start
	bpl.s	stipple.loop

* After the first loop, this code will return to the following routine
* (stipple.other.lines).  After the second loop, return to the caller.
...
stipple.other.lines
	move.l	(sp)+,a0		restore screen pointer
	lea	160(a0),a0
	ror.l	#1,d4			opposite masks for other lines
	ror.l	#1,d5
	ror.l	#1,d6
	ror.l	#1,d7
	move.l	d3,a1			restore fill co-ords pointer
	move.w	(a1)+,d0
	bpl	stipple.loop
	rts
...
* one word fill is handled above
	...
	dc.l	stipple.words2,stipple.words3,stipple.words4
	dc.l	stipple.words5,stipple.words6,stipple.words7
	dc.l	stipple.words8,stipple.words9,stipple.words10
	dc.l	stipple.words11,stipple.words12,stipple.words13
	dc.l	stipple.words14,stipple.words15,stipple.words16
	dc.l	stipple.words17,stipple.words18,stipple.words19
	dc.l	stipple.words20
...
old.stipple.colour
	dc.w	16*4			initially set to colour 1



fill.coords
	ds.w	404	ystart + max. 200 coord pairs + words for end marker
