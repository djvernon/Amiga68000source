...
draw.horizon
	lea	$dff000,a6		clear bitplanes 1 and 3
	blitwait
	move.w	#44,bltdmod(a6)
	move.l	#$1000000,bltcon0(a6)
	move.l	current.scene(pc),bltdpth(a6)
	move.w	#YMAX*2*64+18,bltsize(a6)


	lea	horizon.coords(pc),a0	calculate horizon co-ordinates

	move.w	#-512,d0
	move.w	#512,d1
	move.w	x.angle(pc),d2
	asr.w	#1,d2
...
rotate.horizon
	...
	swap	d1
	add.w	a3,d1
	move.w	d1,(a0)+		rotated y

	dbra	d6,rotate.horizon


	move.w	horizon.coords+8(pc),d0		x1
	move.w	horizon.coords+10(pc),d1	y1
	move.w	horizon.coords+12(pc),d2	x2
	move.w	horizon.coords+14(pc),d3	y2
	bsr.s	bitplane.2.horizon	set bitplane 2


	lea	$dff000,a6		fill bitplane 4
	blitwait
	move.w	#124,bltdmod(a6)
	move.l	#$1ff0000,bltcon0(a6)
	move.l	current.scene(pc),a0
	lea	120(a0),a0
	move.l	a0,bltdpth(a6)
	move.w	#YMAX*64+18,bltsize(a6)


	st	one.bitplane.fill
	move.w	#2*40,fill1.bitplane.offset
	lea	horizon.polygon1(pc),a0
	lea	horizon.coords(pc),a1
	bsr	polygon			set part of bitplane 3


	move.w	#0,fill1.bitplane.offset
	lea	horizon.polygon2(pc),a0
	lea	horizon.coords(pc),a1
	bsr	polygon			set part of bitplane 1
	sf	one.bitplane.fill
	rts


bitplane.2.horizon			* d0 = x1, d1 = y1, d2 = x2, d3 = y2
	move.l	current.scene(pc),a1
...
horizon.coords	ds.w	8*2		x,y pairs

horizon.polygon1
	dc.w	4			number of sides
	dc.w	0,16,20,4
...
fill1.or.4
	tst.b	one.bitplane.fill
	beq	fill.colour

	add.w	fill1.bitplane.offset,a0	to correct bitplane
	moveq	#-1,d4
	bra.s	fill1.loop
...
fill1.loop
	...
	sub.w	d0,d1
	blt.s	next1.line		if x-end is less than x-start
	...
	add.w	d2,d2
	move.w	start1.masks(pc,d2.w),d0	get positive start mask
	...
	add.w	d2,d2
	move.w	end1.masks(pc,d2.w),d2	get positive end mask

	lsr.w	#2,d1			width of fill - 1, in words * 4
	beq.s	one.word.fill1

set.first.words1
	or.w	d0,(a2)+

	move.l	fill1.width.table-4(pc,d1.w),a6
	jmp	(a6)
...
* one word fill is handled above
	...
	dc.l	words1.2,words1.3,words1.4,words1.5,words1.6,words1.7
	dc.l	words1.8,words1.9,words1.10,words1.11,words1.12,words1.13
	dc.l	words1.14,words1.15,words1.16,words1.17,words1.18,words1.19
	dc.l	words1.20
...
fill.colour				* four bitplane fill
	move.w	#0,d1
...
fill.copy.ins
	cmp.w	old.fill4.colour(pc),d1
	beq.s	fill4.loop		if colour has not changed
	move.w	d1,old.fill4.colour

	lea	set.first.words4(pc),a3
	move.w	(a2)+,(a3)+
	move.l	(a2)+,(a3)+
	move.l	(a2)+,(a3)

	lea	set.last.words4(pc),a3
	move.w	(a2)+,(a3)+
	move.l	(a2)+,(a3)+
	move.l	(a2),(a3)
	bra.s	fill4.loop
...
fill4.loop
	move.w	(a1)+,d1		next x-end
	sub.w	d0,d1
	blt.s	next4.line		if x-end is less than x-start
	...
	add.w	d2,d2
	move.w	start4.masks(pc,d2.w),d0	get positive start mask
	...
	add.w	d2,d2
	move.w	end4.masks(pc,d2.w),d2	get positive end mask

	lsr.w	#2,d1			width of fill - 1, in words * 4
	beq.s	one.word.fill4
...
set.first.words4
	...
	move.l	fill4.width.table-4(pc,d1.w),a6
	jmp	(a6)
...
* one word fill is handled above
	...
	dc.l	words4.2,words4.3,words4.4,words4.5,words4.6,words4.7
	dc.l	words4.8,words4.9,words4.10,words4.11,words4.12,words4.13
	dc.l	words4.14,words4.15,words4.16,words4.17,words4.18,words4.19
	dc.l	words4.20
...
;	dc.w	$59e,$58d,$674,$000,$57c,$57c,$56b,$000

	dc.w	$000,$fff,$000,$000,$000,$000,$000,$000
	dc.w	$56b,$57c,$674,$000,$58d,$58d,$59e,$000
