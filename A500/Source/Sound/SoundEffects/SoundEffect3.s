...
vp.wait	move.l	vposr(a6),d0		get vertical beam position
	...
	move.w	#$800f,dmacon(a6)	all sound channels on
...
loop	bsr	frames.per.sec
	...
	lea	sample(pc),a0
	move.w	#8014/2,d0		length
	move.w	#285,d1			period
	move.w	#64,d2			volume
	bsr	sound.effect
...
release.wait	dc.w	0
sample	incbin	StuntCarRacer/sound_data/hit.car.bin
...
next.channel
	...
	moveq	#0,d2
	lea	channel.states(pc),a0
	move.b	(a0,d1.w),d2
	subq.w	#2,d2
	move.b	d2,(a0,d1.w)
	jsr	audio.table(pc,d2.w)

channel.done
	dbra	d1,next.channel
...
audio.error
	addq.w	#1,audio.colour
	move.w	audio.colour(pc),color0(a6)
	bra.s	audio.error
...
; d2.w = volume (0 - 64)
	...
	lea	channel.states+4(pc),a1
	moveq	#3,d3
find.channel
	tst.b	-(a1)			find first free channel
	dbeq	d3,find.channel

	beq.s	channel.found

	move.w	channel.next(pc),d3	get next in line if no channels free
	addq.w	#1,d3
	and.w	#3,d3
	move.w	d3,channel.next
	add.w	d3,a1
