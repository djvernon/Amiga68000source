	section	SoundEffectTest,code_c
	opt	o+,o3-
...
;"""""""""""""
	...
	move.l	#sample,aud0lch(a6)
	move.w	#2436/2,aud0len(a6)
	move.w	#150,aud0per(a6)
	move.w	#30,aud0vol(a6)

	move.w	#0,channel0.state
	move.w	#$8080,intena(a6)	enable channel 0 interrupt
	move.w	#$8001,dmacon(a6)	enable channel 0 DMA
...
; * currently trying to understand why sample plays twice on WinUAE first run

; Audio interrupts occur:-
;  1) At the start of each audio data list - is this the same as 2) below?
;  2) Immediately after AUDxLC and AUDxLEN have been copied into internal registers (by the DMA channel)
;  3) At the end of the sound output cycle i.e. full sample (actually before the last sample is output)

new.level4
	move.w	#$80,intreq(a6)

	cmp.w	#1,channel0.state
;	bne.s	end.level4
	blt.s	end.level4

	move.w	#1,aud0len(a6)
;	move.w	#1,dmacon(a6)		* this isn't correct (stops sample playing twice on first run, but it never plays on subsequent runs)

	cmp.w	#2,channel0.state
	blt.s	end.level4

	move.w	#1,dmacon(a6)		* disabling this makes no difference
;;	move.w	#$80,intena(a6)

end.level4
	addq.w	#1,channel0.state
	rte



channel0.state	dc.w	0
delay	dc.w	0
...
sound.effect
;	cmp.w #3,channel0.state
;	blt.s	.done
	move.w	dmaconr(a6),d0
	and.w	#1,d0
	bne.s	.done

	move.w	#$80,intena(a6)		disable channel 0 interrupt
	move.w	#1,dmacon(a6)		disable channel 0 DMA
...
sample	incbin	StuntCarRacer/sound_data/chime.bin
