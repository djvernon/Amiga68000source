...
x.offset	dc.l	0
y.offset	dc.l	0
z.offset	dc.l	$3000000

new.coords	ds.w	350*3		space for 350 coordinates
...
road.section.x.z.positions	incbin	tracks/track1.xz



far.sections	incbin	tracks/track1.far
end.far.sections

number.of.sections	equ	(*-far.sections)/14



make.road.data
	lea	far.sections(pc),a5
	lea	road.section.x.z.positions(pc),a4
	lea	road+6(pc),a3
	lea	road+6+number.of.sections*2*6(pc),a2
...
make.coords
	...
	ext.w	d6
	asl.w	#2,d6
	asl.w	#8,d6			x position
	sub.w	#256*32,d6

	ext.w	d7
	asl.w	#2,d7
	asl.w	#8,d7			z position
	sub.w	#256*32,d7
	...
	cmp.l	#end.far.sections,a5
	bne.s	make.coords


	move.w	#-6,d0
	move.w	#6,d1
	move.w	#6+number.of.sections*2*6,d2
	move.w	#-6+number.of.sections*2*6,d3

	lea	road+6+number.of.sections*4*6(pc),a5
	move.l	a5,a3
	lea	far.sections(pc),a4
make.right.surfaces
	move.w	#set.fill.colour.offset,(a5)+
	move.w	#fcol15,d7
	move.w	(a4),d6
	btst	#0,d6
	beq.s	right.col.ok
...
right.col.ok
	...
	move.w	#polygon.orientation.offset,(a5)+
	move.w	d0,(a5)+
	move.w	d1,(a5)+
	move.w	d2,(a5)+
	move.w	#2+4*2,(a5)+
	...
	cmp.l	#end.far.sections,a4
	bne.s	make.right.surfaces
	...
	lea	road+6+number.of.sections*(4*6+24)(pc),a5
	move.l	a5,a3
	lea	far.sections(pc),a4
make.top.surfaces
	move.w	#set.fill.colour.offset,(a5)+
...
top.col.ok2
	...
	move.w	#-12+number.of.sections*2*6,d0
	move.w	#0+number.of.sections*2*6,d1
	move.w	#0,d2
	move.w	#-12,d3

	lea	road+6+number.of.sections*(4*6+24*2)+10(pc),a5
	move.l	a5,a3
	lea	far.sections(pc),a4
make.left.surfaces
	move.w	#set.fill.colour.offset,(a5)+
...
left.col.ok
	...
	lea	road+6+number.of.sections*(4*6+24*2)+6(pc),a5
	lea	far.sections(pc),a4
	moveq	#0,d5
make.start.line
	btst	#0,(a4)
	bne.s	found.start.line

	lea	14(a4),a4
	add.w	#2*6,d5
	cmp.l	#end.far.sections,a4
	bne.s	make.start.line
	rts
...
road	dc.w	rotate.coords.offset

	dc.w	0

	dc.w	(number.of.sections*4)-1

; co-ordinates
	ds.w	number.of.sections*4*3

; right surfaces
	ds.w	number.of.sections*12
