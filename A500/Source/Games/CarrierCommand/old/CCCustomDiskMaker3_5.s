...
start	move.w	#$2700,sr		supervisor mode, no interrupts
	...
	jmp	create.start		jump to main code
...
write.bottom.side
	...
	lea	bottomSideData,a0	source		TODO could make pc relative once program relocatable (and if not in separate chipData section)
	bsr	save
...
write.top.side
	...
	lea	topSideData,a0		source		TODO could make pc relative once program relocatable (and if not in separate chipData section)
	bsr	save
...
title.text
	dc.b	SET_COLOUR,5,SET_XY,9,8,'CC Custom Disk Creator'
...
; Source disk data must be in chip RAM for use by blit.code.data
bottomSideData
...
topSideData
	incbin	binary/CCMusic	* length should be 409600
				* no padding required
