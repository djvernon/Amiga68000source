 ...
 * Game load/save - also changed to use 10 sectors per track *
...
color0	equ   $180
	...
	;;Check sizes of new routines copied to $200 and $300
	;;move.l	#g.decode.DOS.track.size,d0	;; size was $a6 (before g.wait.vbl added)
	;;move.l	#g.code.DOS.size,d1		;; size was $ae
	;;rts
...
* Write boot blocks
	...
	lea	boot.blocks(pc),a0
	bsr	code.boot.block

	lea	boot.blocks(pc),a2
	move.l	#1024,d3
	moveq	#0,d4
	bsr	write.to.disk

* Write loading screen data

	moveq	#6-1,d7			6 tracks
	lea	loadscreen,a2
	move.l	#11*512,d3		all sectors of track
	move.l	#1*22*512,d4		start at track 1, bottom side
write.loadscreen
	bsr	write.to.disk
	add.l	d3,a2
	add.l	#22*512,d4		next track on same side
	dbra	d7,write.loadscreen

* Write game data

	bsr	modify.game

	moveq	#59-1,d7		59 tracks
	lea	game(pc),a2
	move.l	#11*512,d3		all sectors of track
	move.l	#7*22*512,d4		start at track 7, bottom side
write.game
	bsr	write.to.disk
	add.l	d3,a2
	add.l	#22*512,d4		next track on same side
...
* Write music data
	...
	moveq	#80-1,d7		80 tracks
	lea	music,a2
	move.l	#10*512,d3		only first 10 sectors of track
	move.l	#(0*22+11)*512,d4	start at track 0, top side
write.music
	bsr	write.to.disk
	add.l	d3,a2
	add.l	#22*512,d4		next track on same side
...
* 28/09/2018 modify load and save routines for real A1200, to allow music to load and saved games to save/load
* modify initDrive wait loop
	move.w	bneIns,$73fa+game-$200	change dbeq to bne

* 04/10/2018 modify setHeadPosition to use wait.vbl (to fix load of saved games in positions 2,3 & 4)
;	move.w	#520*4,$748c+game-$200		;; just quadruple the delay loop: may not work on faster processors
	lea	$748e+game-$200,a1
	move.l	call.wait.vbl(pc),(a1)+
	move.w	nopIns,(a1)+

* 05/10/2018 modify load routine to invalidate head position first (to fix load of saved games in positions 2,3 & 4)
	lea	$749e+game-$200,a1
	move.l	call.invalidate.head(pc),(a1)+

* modify readTrack wait loop
	move.w	beqIns,$752a+game-$200	change dbne to beq
	rts

bneIns	bne	ccopy			(label doesn't matter, as only want opcode word)
beqIns	beq	ccopy			(label doesn't matter, as only want opcode word)
nopIns	nop
...
call.new.code
	jsr	$300.w
call.wait.vbl
	jsr	($200+(g.wait.vbl-g.decode.DOS.track)).w
call.invalidate.head
	jsr	($300+(g.invalidate.head-g.code.DOS.track)).w
...
* 04/10/2018 Wait for vertical blank
...
* 05/10/2018 Invalidate head position, then call initDrive
g.invalidate.head
	st      (g.head.position).w
	jmp	(g.init.drive).w
