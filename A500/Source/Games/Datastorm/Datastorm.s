...
* Datastorm game
* Amiga needs 1MB Chip (ECS Chipset) and 4MB Fast RAM to assemble and run this

* Press F9 to exit during game (also works on most intro screens)

* To do:
* Lines commented with ;? - not sure if address should use label


DISK_ACCESS_DISABLED	equ	1
PROTECTION_DISABLED	equ	1
RELOCATABLE_CODE	equ	1
...
;"""""""""""""""""""""""""
	...
	IFND	DEBUG
	lea	$dff000,a6
	move.w	dmaconr(a6),saved_dmacon	save system DMA status
	move.w	intenar(a6),saved_intena	save system interrupt status

;	move.b	#%00010111,$bfed01	set CIA-A ICR

	sub.l	a0,a0			save exception vectors 0-63
...
origStart
	...
	bra	finish		Datastorm executes rte to return here then finish
...
;	move.w	$dff08a,d0
	...
	move.w	#$9500,$dff09e
;	move.w	#$83d0,$dff096
	move.w	#$8240,$dff096		only enable blitter DMA (not bitplane, copper or disk)
...
* Modify start / size of next RAM area to be used (already populated with data that used to be loaded from disk)
	IFND	RELOCATABLE_CODE
	move.l	#Data200000,$70000	start
	move.l	#Data20000Size,$70004	size
	ELSE
	move.l	#Data200000,L95e	start
	move.l	#Data20000Size,L962	size
	ENDC

	IFD	RELOCATABLE_CODE
* Set bitplane pointers in required copper lists
	move.l	#titleCopperPlanes,a0
	move.l	#Data13,d7
	move.l	#44*240,d6
	jsr	initCopper

	move.l	#specialThanksCopperPlanes,a0
	move.l	#Data26,d7
	move.l	#40*200,d6
	jsr	initCopper
	ENDC
...
.wait	btst	#6,dmaconr(a6)		; wait for blitter to finish
	...
	sub.l	a0,a0			restore exception vectors 0-63
	lea	saved_vectors(pc),a1
	moveq	#64-1,d0
.restore
	move.l	(a1)+,(a0)+
...
;loadSpace	ds.b	$100000-(loadSpace-Data200000)		; make up to 1MB of memory space
Data20000Size	equ	*-Data200000




	section	DScode,code_c

* Datastorm game start
gameStart
;L400	movea.l	#stack,a7
	move.l	#L1700c,L1ad32
...
* Load data area into Chip RAM (copy from expansion RAM if already loaded, otherwise load from disk)
loadData
;L430
	movea.l	#L49e,a0
	jmp	L58e
...
L49e	dc.l	Data0		0 game sprites, radar outline, font
L4a2	dc.l	0
	dc.l	Data1,0		1 hall of fame graphic and running man sprites
	dc.l	Data2,0		2 datascores music
	dc.l	Data3,0		3 game choice screen, 'datastorm' logo (without triangle below)
	dc.l	Data4,0		4 datatitle music
L4c6	dc.l	0,0		5 squid
L4ce	dc.l	0,0		6 skull
	dc.l	Data7,0		7 walker sprite (from attract screens)
	dc.l	Data8,0		8 flags (from attract screens)
	dc.l	Data9,0		9 pilot's hand animation
	dc.l	Data10,0	10 pilot in cockpit
	dc.l	Data11,0	11 cockpit windscreen
	dc.l	Data12,0	12 possibly copper lists and some code
	dc.l	Data13,0	13 title screen with datastorm logo
	dc.l	Data14,0	14 game sound effects
L516	dc.l	0,0		15 possibly mask data
	dc.l	Data16,0	16 'game play' screen 1 (all game play screens are 5 bitplanes)
	dc.l	Data17,0	17 'game play' screen 2
	dc.l	Data18,0	18 'game play' screen 3
	dc.l	Data19,0	19 'game play' screen 4
	dc.l	Data20,0	20 'game play' screen 5
	dc.l	Data21,0	21 'game play' screen 6 (tornado / blitters / mutants)
	dc.l	Data22,0	22 'game play' screen 7 (enemies)
	dc.l	Data23,0	23 'game play' screen 8 (specials)
	dc.l	Data24,0	24 title screen without datastorm logo
	dc.l	Data25,0	25 title screen with ship animation and without datastorm logo
	dc.l	Data26,0	26 title screen ship at full size, with moon
	dc.l	Data27,0	27 ship destroyed (game over) screen, with engine heat animation
	dc.l	Data28,0	28 unknown data (not code)
	dc.l	Data29,0	29 unknown data (not code)
...
L600	; code loaded by this routine (to $4b314) isn't used
	movea.l	#L4b314,a3
	move.l	#$27cc,d5
	move.l	#$638,d6
	...
	adda.l	#$27cc,a2
	move.l	a2,Lc6a
	move.l	#$4b322,$84		trap 1, never used
	st	L10876
	movem.l	(a7)+,d0-d7/a0-a1/a3-a4
	rts
...
L7dc	dc.l	$00002c00,DATA0SIZE
	dc.l	$0000c400,DATA1SIZE
	dc.l	$00014600,DATA2SIZE
	dc.l	$0001d000,DATA3SIZE
	dc.l	$00022800,DATA4SIZE
	dc.l	$00046000,DATA5SIZE
L80c	dc.l	$00047400
L810	dc.l	DATA6SIZE
	dc.l	$0004ae00,DATA7SIZE
	dc.l	$0004c000,DATA8SIZE
	dc.l	$0004f400,DATA9SIZE
	dc.l	$0005de00,DATA10SIZE
	dc.l	$00062800,DATA11SIZE
	dc.l	$00063c00,DATA12SIZE
	dc.l	$00064400,DATA13SIZE
	dc.l	$0006d200,DATA14SIZE
	dc.l	$00093400,DATA15SIZE
	dc.l	$00093e00,DATA16SIZE
	dc.l	$00094e00,DATA17SIZE
	dc.l	$00095a00,DATA18SIZE
	dc.l	$00097200,DATA19SIZE
	dc.l	$00098200,DATA20SIZE
	dc.l	$00098a00,DATA21SIZE
	dc.l	$00099200,DATA22SIZE
	dc.l	$00099e00,DATA23SIZE
	dc.l	$0009aa00,DATA24SIZE
L8a4	dc.l	$000a2e00
L8a8	dc.l	DATA25SIZE
	dc.l	$000b8e00,DATA26SIZE
	dc.l	$000bae00,DATA27SIZE
	dc.l	$000c6000,DATA28SIZE
	dc.l	$000c6200,DATA29SIZE
...
* Load data areas from disk to expansion RAM, until full
L966
	moveq	#0,d0
L968
	lea	L7dc(pc),a0
...
Ld32
	...
	move.w	#1,L1698
	move.w	#1,L1ad30
	move.l	#L74000,L133a6
	jsr	L14ac4
	jsr	L14b9e
	move.l	#L6c9d2,L133a6
	move.w	#1,L1698
	move.w	#1,L1ad30
	jsr	L16784
	jsr	L16932
	move.l	#L64c70,L133a6
	jsr	L16a2
	jsr	Le9e
	jsr	L16a2
	move.l	#L17916,L133a6
	move.w	#$8400,$dff096
	jmp	Lf92


Ldf4
	move.l	#L76000,L133a6
	jsr	L15310
	jsr	L15404
	jmp	Ld32
...
Le10
	...
	jsr	L1576c
	jsr	L15876
	tst.w	pilotAnimationFlag
	beq.s	Le74
	jsr	L161d2
	jsr	L162b8
Le74
	move.l	#L4e4a0,L133a6
	clr.w	L1698
	clr.w	L1ad30
	jsr	Le9e
	move.w	#$a,L169e
...
Le9e
	...
	moveq	#14,d0
	jsr	loadData
	movea.l	#Data14+DATA14SIZE,a0
	movea.l	#Data14,a1
	movea.l	#Data14End,a2
	jsr	decompress
	move.l	#L44d76,L516
	move.l	#L44d76,L4136
	bra.s	Lf18
...
* Main loop during game and intro screens that have same backdrop (ground / radar)
Lf92
	jsr	swapScreens		wait for next frame signal (two copper interrupts i.e. run at 25Hz)
	jsr	animatePortal
	move.w	L562e,Ledd2
	jsr	clearGameDisplay
	jsr	updateGroundColours
	jsr	increaseLives
	jsr	L10410			no pods, aliens or player movement without
	jsr	L11b30
	jsr	La26c
	jsr	L4dac
	jsr	L11db0
	jsr	L11dfc
	jsr	L127ae
	jsr	Le952			possibly clear collision map memory
	jsr	drawBackgroundDots	(stars)
	jsr	drawExplosionDots
	jsr	drawAppearDots
	jsr	alienBullets
	jsr	L727c			no player movement, gravity or firing without
	jsr	L74cc
	jsr	L4e74
	jsr	clearRadarDisplay
	jsr	L95be
	jsr	drawPlayerShip
	jsr	L7b74
	jsr	L9c12			no player bullets or bullet collision detection without
	jsr	drawPlayerShield
	jsr	L9eb0			no player bullets or bullet collision detection without
	jsr	podDropOff
	jsr	L76f2			no scrolling without
	jsr	handlePods
	jsr	L4fdc
	jsr	L7d92			pods not collectable without
	jsr	L80e2
	jsr	drawRadar
	jsr	L10d2a			no aliens without
	jsr	L11414
	jsr	playerExplosion
	jsr	L5630
	jsr	L65c2
...
L120a
	moveq	#13,d0
	jsr	loadData
	movea.l	#Data13+DATA13SIZE,a0
	movea.l	#Data13,a1
	movea.l	#Data13End,a2
	jsr	decompress
	move.l	#titleCopper,Lfcf8
	jsr	Lfcfc
	jsr	Lfef8
	jsr	Lff1e
	jmp	$24
...
L1278
	...
	moveq	#24,d0
	jsr	loadData
	movea.l	#Data24+DATA24SIZE,a0
	movea.l	#Data24,a1
	movea.l	#Data24End,a2
	jsr	decompress
	move.l	#Lf666,Le53e
	move.l	#Le128,$6c
	move.w	#$8030,$dff09a
	...
	moveq	#25,d0
	jsr	loadData
	movea.l	#Data25+DATA25SIZE,a0
	movea.l	#Data25,a1
	movea.l	#Data25End,a2
	jsr	decompress
L130a
	move.b	$bfe001,d0
	btst	#7,d0
...
* Sprite pointers and details?
L14c6	dc.w	$0000,$00d0,$0000,$0062
	dc.l	$000009f4,L6111a
	dc.w	$0000,$00c0,$0000,$005a
	dc.l	$00000870,L642de
	dc.w	$0000,$00b0,$0000,$0052
	dc.l	$0000070c,L66d0e
	dc.w	$0000,$00a0,$0000,$004b
	dc.l	$000005dc,L6904a
	dc.w	$0000,$0090,$0000,$0045
	dc.l	$000004da,L6ad96
	dc.w	$0000,$0080,$0000,$003f
	dc.l	$000003f0,L6c5d8
	dc.w	$0000,$0080,$0000,$003a
	dc.l	$000003a0,L6d988
	dc.w	$0000,$0070,$0000,$0035
	dc.l	$000002e6,L6eba8
	dc.w	$0000,$0070,$0000,$0031
	dc.l	$000002ae,L6fa26
	dc.w	$0000,$0060,$0000,$002d
	dc.l	$0000021c,L7078c
	dc.w	$0000,$0060,$0000,$0028
	dc.l	$000001e0,L71218
	dc.w	$0000,$0050,$0000,$0025
	dc.l	$00000172,L71b78
	dc.w	$0000,$0050,$0000,$0022
	dc.l	$00000154,L722b2
	dc.w	$0000,$0040,$0000,$001f
	dc.l	$000000f8,L72956
	dc.w	$0000,$0040,$0000,$001d
	dc.l	$000000e8,L72e2e
	dc.w	$0000,$0040,$0000,$001a
	dc.l	$000000d0,L732b6
	dc.w	$0000,$0030,$0000,$0018
	dc.l	$00000090,L736c6
	dc.w	$0000,$0030,$0000,$0014
	dc.l	$00000078,L73996
	dc.w	$0000,$0030,$0000,$0010
	dc.l	$00000060,L73bee
	dc.w	$0000,$0020,$0000,$000e
	dc.l	$00000038,L73dce
	dc.w	$0000,$0020,$0000,$000b
	dc.l	$0000002c,L73ee6
	dc.w	$0000,$0020,$0000,$000a
	dc.l	$00000028,L73fc2
	dc.w	$0000,$0020,$0000,$0008
	dc.l	$00000020,L7408a
	dc.w	$0000,$0010,$0000,$0007
	dc.l	$0000000e,L7412a
	dc.w	$0000,$0010,$0000,$0005
	dc.l	$0000000a,L74170


L1656
	move.w	Le124,d1
	move.b	$bfec01,d0
	cmpi.b	#$cd,d0		P pressed
	bne.s	L168c
L1668
	move.b	$bfec01,d0
	cmpi.b	#$cd,d0		P pressed
...
L1936
	dc.w	$0005,$0096,$009b,$0140,$0145,$0195,$019a,$0226,$022b,$02b2,$02b7,$035c,$0366,$0406
...
L1e1a
	...
	move.w	#$160,L1338a
	move.w	#$98,L1338c
	move.l	#L4749a,L1337a
	move.l	#L6fbb6,L1338e
	move.w	#$ca,L13396
	move.w	#$37,L13398
	move.w	#$30,L1339a
	move.w	#4,L1339c
...
L1ec8
	...
	moveq	#16,d0
	jsr	loadData
	movea.l	#Data16+DATA16SIZE,a0
	movea.l	#Data16,a1
	movea.l	#Data16End,a2
	jsr	decompress
	addq.w	#1,L169e
	clr.w	Lb8c8
	addi.w	#$2f6,L7268
...
L1f48
	moveq	#17,d0
	jsr	loadData
	movea.l	#Data17+DATA17SIZE,a0
	movea.l	#Data17,a1
	movea.l	#Data17End,a2
	jsr	decompress
	addq.w	#1,L169e
	clr.w	Lb8c8
	subi.w	#$c8,L7268
...
L1fac
	moveq	#18,d0
	jsr	loadData
	movea.l	#Data18+DATA18SIZE,a0
	movea.l	#Data18,a1
	movea.l	#Data18End,a2
	jsr	decompress
	addq.w	#1,L169e
	clr.w	Lb8c8
	movea.l	#L2380,a0
...
L1ff4
	moveq	#19,d0
	jsr	loadData
	movea.l	#Data19+DATA19SIZE,a0
	movea.l	#Data19,a1
	movea.l	#Data19End,a2
	jsr	decompress
	addq.w	#1,L169e
	addi.w	#$145,L7268
	addi.w	#$145,L74b8
...
L2054
	moveq	#20,d0
	jsr	loadData
	movea.l	#Data20+DATA20SIZE,a0
	movea.l	#Data20,a1
	movea.l	#Data20End,a2
	jsr	decompress
	addq.w	#1,L169e
	clr.w	Lb8c8
	subi.w	#$145,L7268
...
L20b8
	moveq	#21,d0
	jsr	loadData
	movea.l	#Data21+DATA21SIZE,a0
	movea.l	#Data21,a1
	movea.l	#Data21End,a2
	jsr	decompress
	addq.w	#1,L169e
	move.w	#1,L21a2
	jsr	L11c32
...
L2102
	moveq	#22,d0
	jsr	loadData
	movea.l	#Data22+DATA22SIZE,a0
	movea.l	#Data22,a1
	movea.l	#Data22End,a2
	jsr	decompress
	addq.w	#1,L169e
	clr.w	Lb8c8
	movea.l	#L2470,a0
...
L21a2	dc.w	$0000
L21a4	dc.w	$001f,$0000,$0021,$0000,$0000,$0008,$001f,$0000,$0031,$0000,$0000,$0001,$001b,$0000,$0041
	...
	dc.w	$0000,$0082,$0000,$0000,$000d,$0000,$0000

L2250	dc.w	$001f,$0000,$0021,$0000,$0000,$001b,$0020,$0000,$0031
...
L2308	dc.w	$0020,$0000,$0012,$0000,$0000,$0018,$0020,$0000,$0027,$0000,$0000,$001a,$0020
	dc.w	$0000,$003c,$0000,$0000,$002f,$0020,$0000,$0051,$0000,$0000,$0019,$0020,$0000,$0074,$0000,$0000
	dc.w	$002a,$0000,$0000

L2348	dc.w	$0338,$0000,$002e,$0000,$0000,$0010,$0401,$0000,$004b,$0000,$0000,$0011,$0000,$0000

L2364	dc.w	$02c4,$0000,$003f,$0000,$0000,$0010,$0306,$0000,$0064,$0000,$0000,$0011,$0000,$0000
...
L23e4	dc.w	$03ec,$0000,$0018,$0000,$0000,$0010,$0487,$0000,$0048,$0000,$0000,$0011,$03f0,$0000,$007f
	...
	dc.w	$0001,$03b3,$0000,$008a,$0000,$0000,$0001,$040f,$0000,$0088,$0000,$0000,$0001,$0000,$0000

L2460	dc.w	$028e
	dc.w	$0000,$0040,$0000,$0000,$0001,$0000,$0000

L2470	dc.w	$0248,$0000,$002c,$0000,$0000,$001b,$0248,$0000,$0040
...
L24ec	dc.w	$028c,$0000,$003e,$0000,$0000,$0010,$0294,$0000,$003e,$0000,$0000
...
L25b0	dc.b	$0d,'w',$0c,$02,'d a t a s c o r e s',$0d,'?',$15,$03,'lander ',$01,'50 pts',$0d,'?',$0f,$02,'crazy ',$01,'150 pts',$0d,'?',$0f,$04,'t'
	dc.b	'ornado ',$01,'500 pts',$0d,'?',$0e,$04,'blitter ',$01,'75 pts',$0d,'?',$0f,$05,'mutant',$01,' 75 pts',$0d,'?',$0f,$05,'mutant',$01,' '
	dc.b	'75 pts',$0d,'?',$0f,$05,'mutant',$01,' 75 pts',$00

L2649	dc.b	$0d,$d4,'"',$03,'gravitron',$01,' 100 pts',$0d,$d4,$0f,$03,'pulsar',$01,' 50 pt'
	dc.b	's',$0d,$d4,$0f,$04,'saturn',$01,' 200 pts',$0d,$d4,$0f,$04,'drone',$01,' 50 pts',$0d,$d4,$0f,$02,'baiter',$01,' 200 pts',$0d,$d4,$0f,$03,'egg',$01
	dc.b	' 50 pts',$0d,$d4,$0f,$05,'wall',$01,' 25 pts',$00

L26c8	dc.b	$0d,'w',$0c,$02,'d a t a s c o r e s',$0d,'?',$15,$02,'bipod',$01,' 100 pt'
	dc.b	's',$0d,'?',$0f,$03,'spiker',$01,' 100 pts',$0d,'?',$0f,$03,'twirler',$01,' 50 pts',$0d,'?',$0f,$04,'rocket',$01,' 50 pts',$0d,'?',$0f,$04,'bla'
	dc.b	'st',$01,' 50 pts',$0d,'?',$0f,$02,'tanker',$01,' 250 pts',$0d,'?',$0f,$03,'bomber',$01,' 75 pts',$00

L2760	dc.b	$0d,$d4,'"',$02,'hunter',$01,' 100 '
	dc.b	'pts',$0d,$d4,$0f,$05,'fireball',$01,' 10 pts',$0d,$d4,$0f,$05,'mc squid',$01,' 10 pts',$0d,$d4,$0f,$04,'pod',$01,' 1/2/4/800 pt'
	dc.b	's',$0d,$d4,$0f,$04,'traitor',$01,' 100 pts',$0d,$d4,$0f,$03,'man',$0d,$d4,$0f,$03,'zomboid',$01,' ? pts',$00

L27df	dc.b	$0d,'w',$05,$02,'d a t a s c o'
	...
	dc.b	'omb ',$04,'has limited effect on space squid',$00

L28d7	dc.b	$0d,'w',$05,$02,'d a t a s c o r e s',$0d,$82
	...
	dc.b	'mited effect',$0d,$aa,$07,' on space skull',$00

L298f	dc.b	$0d,$91,$03,$02,'p o w e r s',$0d,'?',$0e,$03,'lazer ',$01,'arms yo'
	...
	dc.b	$0d,']',$06,'the current level',$0d,'?',$0d,$04,'collect powers by flying into them!',$00

L2bac	dc.b	$0d,'w',$0c,$02
	dc.b	'd a t a s c o r e s',$0d,$87,'2',$03,'u.f.o. ',$01,'500 pts',$0d,'d1',$02,'smartbomb ',$04,'has limited'
	dc.b	' effect',$0d,$93,$07,' on u.f.o.',$00

L2c05	dc.b	$0d,'k',$0c,$04,'i n s t r u c t i o n s',$0d,'"',$14,$01,'using the jo'
...
L2db9	dc.b	$0d,'k',$0c,$04,'i n s t r u c t i o n s',$0d,'"',$14,$01,'by pressing the spacebar'
...
L337e
	...
	move.w	#4,player1Lives
	move.w	#3,player1SmartBombs
	clr.w	player1Missiles
	move.w	#$21,player1Shield
	clr.w	player1ShieldOn
	clr.w	L7260
	clr.w	L7262
	clr.w	L7264
	move.l	#$a00000,L7268
	...
	clr.l	L7270
	clr.l	L7274
	move.w	#1,player1Lasers
	clr.w	player1AutofireRemaining
	clr.w	L727a
	...
	clr.l	L74c0
	clr.l	L74c4
	move.w	#1,player2Lasers
	clr.w	player2AutofireRemaining
	clr.w	L74ca
...
L3abe
	...
	move.w	#$26,$dff066
	move.w	#$184,d7
	move.l	#L648b0,$dff050
	movea.l	radarDisplayMem,a2
	adda.l	#$1a2,a2
	move.w	#$9f0,$dff040
	moveq	#4,d2
L3b22
...
L3c14
	...
	dc.w	$047b,$7c7b,$7c7b,$7c00

L3c3c
	dc.w	$0320,$2020,$2020,$2000,$0320,$2020,$2020,$2000,$037b,$7c20,$2020,$2000
	dc.w	$037b,$7c7b,$7c20,$2000,$037b,$7c7b,$7c7b,$7c00

L3c64
	dc.w	$0220,$2020,$0002,$7b20,$2000,$027b,$7b20,$0002,$7b7b,$7b00

L3c78
	dc.w	$0520,$2020,$0005,$7b20,$2000,$057b,$7b20,$0005,$7b7b,$7b00


L3c8c
	clr.l	d0
	move.w	player1Lives,d0
	cmpi.w	#3,L103ea
	bne.s	L3ca0
	addq.w	#1,d0
L3ca0
...
L3f16
	...
	dc.w	$ffff,$ffff,$a000,$0000,$ffff,$ffff,$e000,$0000

L4026
	dc.w	$8800,$0000,$0200,$0000,$8c00,$0000,$0200,$0000
...
drawPlayerShield	; also draw shield and autofire bars next to radar
	tst.w	L103ec
	bne.s	L4188
	move.w	L727a,d0
	andi.w	#$fffc,d0
...
L4170
	...
	jsr	L1ad18
L4188
	move.w	player1AutofireRemaining,d2
	lsr.w	#4,d2
	mulu.w	#$14,d2
	movea.l	L4136,a0
	adda.l	d2,a0
...
L4294
	...
	dbra	d2,L4294
L42ce
	move.w	player2AutofireRemaining,d2
	lsr.w	#4,d2
	mulu.w	#$14,d2
	movea.l	L4136,a0
	adda.l	d2,a0
...
L4938
	clr.l	d0
	move.w	player1Shield,d0
	asl.w	#3,d0
	addi.l	#L3f16,d0
	movea.l	d0,a0
	movea.l	radarDisplayMem,a1
	adda.l	#$1700,a1
	adda.l	#$426,a1
	move.l	(a0),(a1)
	move.l	(a0)+,$2e(a1)
...
L49b2
	...
	bpl.s	L4a44
	move.b	#$28,L413b
	tst.w	player1ShieldOn
	beq.s	L4a0c
	IFND	INFINITE_SHIELD
	subq.b	#1,player1Shield+1
	ELSE
	nop
	nop
	nop
	ENDC
	cmpi.b	#2,player1Shield+1
	bne.s	L49ee
	moveq	#$22,d0
	jsr	L1ad18
L49ee
	tst.b	player1Shield+1
	bne.s	L4a0c
	clr.w	player1ShieldOn
	cmpi.w	#3,L103ea
	bne.s	L4a0c
...
L4ab8
	tst.w	player1Shield
	beq.s	L4ac8
	eori.w	#1,player1ShieldOn	shield on
L4ac8
	rts
...
L4bd0
	dc.w	$0000
L4bd2
	dc.w	$2020,$2020,$2520,$2020,$2525,$2020,$2525,$2520,$2525,$2525
...
L6bba	dc.w	$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000
	dc.w	$0007,$0014,$0020,$002b,$0039,$0007,$0014,$0020,$002b,$0039,$0007,$0014,$0020,$002b,$0039

L6bf6	dc.w	$002c,$002c,$002c,$002c,$002c,$004d,$004d,$004d,$004d,$004d,$0068,$0068,$0068,$0068,$0068,$0000,$0000
	dc.w	$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000

L6c32
	dc.w	$fffc,$ffff,$0000,$0001,$0004
L6c3c
	dc.w	$fffc,$fffe,$0001,$0003,$0006
L6c46
	dc.w	$0000
...
increaseLives	; and smart bombs every 10000 pts
;L706a
	tst.w	L7266
	beq.s	L70be
	move.b	L37d5,d0
	...
	move.b	#$30,L7068
L7098
	addq.w	#1,player1Lives
	addq.w	#1,player1SmartBombs
	jsr	L3c8c
	moveq	#$12,d0
	jsr	L1ad18
	move.l	#Le250,Le24c
...
player1Shield
;L725c
	dc.b	$00,$21		shield remaining (max = $21)

player1ShieldOn
;L725e
	dc.w	$0001		shield on flag (0 or 1)
...
L7264
	dc.w	$0000
L7266
	dc.w	$0001		cleared when player's game is over
L7268
	dc.w	$00a0,$0000
L726c
	dc.w	$0000,$0000
...
L78f2
	dc.l	L6fca6,L6fdbe,L6fed6
L78fe	dc.l	L6fd32,L6fe4a,L6ff62
L790a	dc.l	L6ffee,L70106,L7021e
L7916	dc.l	L7007a,L70192,L702aa
L7922	dc.l	L70336,L703ea,L7049e,L703ea
L7932	dc.l	L70552,L70606,L706ba,L70606
L7942
	dc.b	$00
L7943
	dc.b	$04
...
L7e80
	...
	cmpi.w	#3,L103ea
	beq.s	L7ed2
	cmpi.w	#1,player1Lives
	bne.s	L7ed2
	jsr	L7142
	move.b	#1,d0
	jsr	L11de8
L7ed2
	cmpi.w	#1,player1Lasers
	IFND	KEEP_LASERS
	beq.s	L7ee2
	subq.w	#1,player1Lasers
	ENDC
L7ee2
	clr.w	L9fd4
	move.l	#Le32e,Le248
...
L7fd2
	...
	clr.b	1(a5)
	clr.b	d0
	cmpi.w	#3,player1Lasers
	beq.s	L7ff6
	st	d0
	addq.w	#1,player1Lasers
L7ff6
	subq.w	#1,L10aa8
	tst.b	d0
	bne.s	L801e
...
L807e
	...
	clr.b	(a5)
	clr.b	1(a5)
	move.w	#$208,player1AutofireRemaining
	subq.w	#1,L10aa8
	moveq	#$1a,d0
	jsr	L1ad18
	jmp	La04
...
L81d0
	...
	jsr	L11de8
L8222
	cmpi.w	#1,player2Lasers
	beq.s	L8232
	subq.w	#1,player2Lasers
L8232
	clr.w	L9fd6
	move.l	#Le32e,Le248
	tst.w	L74b0
...
L8322
	...
	clr.b	1(a5)
	clr.b	d0
	cmpi.w	#3,player2Lasers
	beq.s	L8346
	st	d0
	addq.w	#1,player2Lasers
L8346
	subq.w	#1,L10aa8
	tst.b	d0
	bne.s	L836e
...
L83ce
	...
	clr.b	(a5)
	clr.b	1(a5)
	move.w	#$208,player2AutofireRemaining
	subq.w	#1,L10aa8
	moveq	#$1a,d0
	jsr	L1ad18
	rts
...
player2Lasers
;L9bf2
	dc.w	$0001		; probably player2 lasers

player1AutofireRemaining
;L9bf4
	dc.w	$0000
player2AutofireRemaining
;L9bf6
	dc.w	$0000

L9bf8	dc.w	$ba54,$c05f,$ffa0,$ffff,$ffff,$2a50,$fa03,$05ff,$ffff,$ffff
L9c0c	dc.w	$0000
L9c0e	dc.w	$0000,$0000


L9c12
	tst.w	player1AutofireRemaining
	beq.s	L9c22
	tst.w	L9fdc
	bne.s	L9c32
L9c22
...
L9c48
	tst.w	player1AutofireRemaining
	IFND	INFINITE_AUTOFIRE
	beq.s	L9c56
	subq.w	#1,player1AutofireRemaining
	ENDC
L9c56
	moveq	#1,d0
	jsr	L1ad18
...
L9d58
	...
	dbra	d7,L9ce6
L9d5e
	tst.w	player2AutofireRemaining
	beq.s	L9d6e
	tst.w	L9fe0
	bne.s	L9d7e
L9d6e
...
L9d94
	tst.w	player2AutofireRemaining
	beq.s	L9da2
	subq.w	#1,player2AutofireRemaining
L9da2
	move.l	#1,d0
	jsr	L1ad18
	cmpi.w	#1,player2Lasers
...
La1aa
	dc.w	$0000
La1ac
	dc.l	Lb7cc,La352,La49c,La500,La652,La818,La8b6,La940
	dc.l	La992,Laaf8,Laaf8,Laaf8,Lac3e,Lacca,Lace6,Laebe
	dc.l	Laee8,Laee8,Laee8,Laee8,Laee8,Laee8,Laee8,Laee8
	dc.l	Lafa0,Lafa0,Lafa0,Laff4,Lb076,Lb0c8,Lb1ca,Lb2ce
	dc.l	Lb348,Lb7cc,Lb372,Lb422,Lb47c,Lb4e8,Lb572,Lb592
	dc.l	Lb5dc,Lb5f2,Lafa0,Lb5dc,Lb66c,Lb784,Lb784,Lafa0
...
Lcba4
	dc.l	0,L6a300,L6b200,L6b700,L69e00,L6bf20,L6bc00,L6b480
	dc.l	L69e00,L6a440,L6a580,L6a6c0,L6c790,L6a080,L6d820,0
	dc.l	L6c600,L6c600,L6c600,L6c600,L6c600,L6c600,L6c600,L6c600
	dc.l	L6d780,L6d5a0,L6d500,L6a800,L6da00,L6ad00,L6aee0,L6c100
	dc.l	L6cb00,L6bfc0,L6d3c0,L6d8c0,L6cf60,L6b160,L6db90,L6dcd0
	dc.l	L6df00,L6d000,L6d640,L6e040,L6c920,L6e2c0,L6e400,L6d6e0
...
Lcd26	dc.w	$0000,$000c,$0003,$0000,$0000,$0000,$0000,$0000,$0000,$0050,$0003,$0000,$0000,$0000,$0000
	...
	dc.w	$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000
Lcde4
	dc.l	$00000,L1670e,L16704,L166e6,L16722,L16736,L166fa,L166e6
	dc.l	L166e6,L16704,L166dc,L166d2,L166d2,L166d2,L166d2,L166d2
	dc.l	L166fa,L166fa,L166fa,L166fa,L166fa,L166fa,L166fa,L166fa
	dc.l	L16736,L16736,L16736,L16704,L16718,L166e6,L16718,L16704
	dc.l	L166d2,L166e6,L166e6,L166e6,L166e6,L16722,L166c8,L166fa
	dc.l	L166d2,L16704,L166d2,L166d2,L16704,L166d2,L166d2,L166d2
Lcea4
	dc.w	$0000
Lcea6
	dc.l	0
...
handlePods	; pods not visible or collectable without
;Lceac
	addq.b	#1,Lcb99
	addq.b	#1,Lcba3
	addq.b	#1,Lcb75
...
drawExplosionDots	; excluding player explosion
;Ld6a4
	movea.l	gameDisplayMem,a1
	movea.l	#Lffea,a3
	move.w	Le126,d3
...
Le0ee
	dc.w	$ffff,$0000,$0001
Le0f4
	dc.w	$0000,$8000,$0001,$0000,$0002,$0000
Le100
	dc.w	$0000,$2800,$0000,$5000,$0000,$a000
Le10c
	dc.w	$ffff,$e800,$ffff,$d000,$ffff,$a000
Le118
	dc.w	$0000,$1000,$0000,$2000,$0000,$3000
Le124
	dc.b	$00
Le125
	dc.b	$00
...
* Level 3 interrupt 1, used during game and intro screens (except hall of fame)
Le128
	movem.l	d0/a0,-(a7)
	move.w	$dff01e,d0
	btst	#4,d0
...
* Added by DV
	move.b	$bfec01,d0
	cmpi.b	#$4f,d0		F9 pressed
	bne.s	.continue
	move.l	saved_a7,a7
	rte			exit Datastorm by returning to the line before label trap0
.continue
* End of addition
	addq.b	#1,Le162
	movea.l	Le248,a0
...
* Level 3 interrupt 2, used during hall of fame
Le1c4
	movem.l	d0-d1/a0,-(a7)
	move.w	$dff01e,d0
	btst	#4,d0
...
Le250
	dc.w	$000f,$000e,$000d,$000c,$000b,$000a,$0009,$0008,$0007,$0006,$0005,$0004,$0003,$0002,$0001,$0000,$ffff

Le272
	dc.w	$00f0,$00e0,$00d0,$00c0,$00b0,$00a0,$0090,$0080,$0070,$0060,$0050,$0040,$0030,$0020,$0010,$0000,$ffff

Le294
	dc.w	$0800,$0700,$0600,$0500,$0400,$0300,$0200,$0000,$ffff
Le2a6
	dc.w	$0fff,$0eef,$0ddf,$0ccf,$0bbf,$0aaf,$099f,$088f,$077f,$066f,$055f,$044f,$033f,$022f,$011f,$000e
	dc.w	$000d,$000c,$000b,$000a,$0009,$0008,$0007,$0006,$0005,$0004,$0003,$0002,$0001,$0000,$ffff

Le2e4
	dc.w	$0fff,$0efe,$0dfd,$0cfc,$0bfb,$0afa,$09f9,$08f8,$07f7,$06f6,$05f5,$04f4,$03f3,$02f2,$01f1,$00e0
	dc.w	$00d0,$00c0,$00b0,$00a0,$0090,$0080,$0070,$0060,$0050,$0040,$0030,$0020,$0010,$0000,$ffff

Le322
	dc.w	$0550,$0440,$0330,$0220,$0000,$ffff
Le32e
	dc.w	$0fff,$0eee,$0ddd,$0ccc,$0bbb,$0aaa,$0999,$0888,$0777,$0666,$0555,$0444,$0333,$0222,$0111,$0000,$ffff
Le350
	dc.w	$0f00,$0000,$0d00,$0000,$0b00,$0000,$0900,$0000,$0700,$0000,$0500,$0000,$0300,$0000,$0200,$0000,$ffff
Le372
	dc.w	$0fff,$0333,$0fff,$0333,$0fff,$0333,$0fff,$0333,$0fff,$0333,$0fff,$0333,$0fff
...
Le542
	...
	dc.w	$0000
Le548
	dc.l	L57fca,L4f73c,L5128c,L52ddc,L5492c,L5647c,L4f73a,L57fcc,L59b1c,L5b66c,L5d1bc,L5ed0c
Le578
	dc.l	L6251a,L6085c,L60e1c,L613dc,L6199c,L61f5c,L6085a,L6251c,L62adc,L6309c,L6365c,L63c1c
Le5a8
	dc.l	L583da,L4d02c,L4f41c,L5180c,L53bfc,L55fec,L4d02a,L583dc,L5a7cc,L5cbbc,L5efac,L6139c
Le5d8
	dc.l	L5aaea,L4f73c,L51b2c,L53f1c,L5630c,L586fc,L4f73a,L5aaec,L5cedc,L5f2cc,L616bc,L63aac
Le608
	dc.l	L5a466,L4d626,L4ff66,L528a6,L551e6,L57b26,L4d626,L5a466,L5cda6,L5f6e6,L62026,L64966
Le638
	dc.l	L50c32,L4f73a,L5212a,L53622,L5b082,L54b1a,L55f62,L573aa,L587f2,L59c3a,L4f73a,L50c32
	dc.l	L5212a,L53622,L54b1a,L5b082,L5c4ca,L5d912,L5ed5a,L601a2
Le688
	dc.l	L6ca7a,L5fc3a,L6257a,L64eba,L677fa,L6a13a,L5fc3a,L6ca7a,L6f3ba,L71cfa,L7463a,L76f7a
Le6b8
	dc.l	L542da,L4749a,L49dda,L4c71a,L4f05a,L5199a,L4749a,L542da,L56c1a,L5955a,L5be9a,L5e7da
...
Le6e8
	...
	adda.w	Le534,a0
	adda.w	Le534,a2
	move.l	(a0)+,gameDisplayMem
	move.l	gameDisplayMem,$8
	move.l	(a2)+,radarDisplayMem
	movea.l	Le53e,a1
	movea.l	Le542,a3
	moveq	#9,d0
Le72c
...
;Le902
	...
	move.w	#22,$dff066
	moveq	#4,d1
	move.l	radarDisplayMem,d0
	addi.l	#3*46+12,d0
Le932
	jsr	blitWait
	move.l	d0,$dff054
	move.w	#19*64+12,$dff058
	addi.l	#32*46,d0
	dbra	d1,Le932
	rts
...
updateGroundColours	; possibly just colour fade routine, used for more than just the ground
;Leace
	tst.w	Leaba
	beq	Leb86
	move.w	Leaba,d0
...
Leb8c	dc.w	$0411,$0fc0,$0eb0,$0d91,$0c81,$0c71,$0b61,$0a51,$0952,$0842,$0732,$0632,$0621,$0521,$0411,$0311
Lebac	dc.w	$0003,$0fff,$0dde,$0bbd,$099c,$088b,$066a,$0559,$0448,$0337,$0226,$0115,$0114,$0003,$0002,$0001
Lebcc	dc.w	$0300,$0fee,$0ecc,$0daa,$0c99,$0b77,$0a66,$0955,$0844,$0733,$0622,$0511,$0411,$0300,$0200,$0100
Lebec	dc.w	$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000
...
Lecf0
	...
	dc.w	$013c,$0000,$013e,$0000,$ffff,$fffe

Led3c	dc.w	$0100,$5200,$0104,$0000,$008e,$2a71,$0090,$f4d1,$0092,$0030
...
Ledd2
	...
	dc.w	$0028,$0108,$00fc,$010a,$00fc,$0102
Lee5e
	dc.w	$0000,$0180
Lee62
	...
	dc.w	$0000,$01be,$0000,$01b0,$0000
Leeec
	dc.w	$00e0,$0005,$00e2,$0000,$00e4,$0005,$00e6,$1280,$00e8,$0005,$00ea
	dc.w	$2500,$00ec,$0005,$00ee,$3780,$00f0,$0005,$00f2,$4a00,$c301,$ff00,$0100,$5200,$0180,$0000,$d301
	dc.w	$ff00,$0100,$0200,$0092,$0030,$0108,$0002,$010a,$0002,$0102,$0000,$00e0
Lef3a	dc.w	$0000,$00e2,$0000,$00e4
...
Lef62
	...
	dc.w	$ff00,$5100,$5200,$d601,$ff00,$009c,$8010,$ffff,$fffe

Leff4
	dc.w	$0100,$5200,$0104,$0000,$008e,$2a71,$0090,$f2d1,$0092,$0030,$0094,$00d8,$0108,$0002,$010a,$0002,$00e0
Lf016
	dc.w	$0000,$00e2,$0000,$00e4,$0000,$00e6
...
specialThanksCopper
;Lf0c8
	dc.w	$0100,$5200,$0104,$0000,$008e,$2c81,$0090,$f4c1,$0092,$0038,$0094,$00d0,$0108,$0000,$010a,$0000
specialThanksCopperPlanes
	dc.w	$00e0,$0005,$00e2,$0000
	dc.w	$00e4,$0005,$00e6,$1f40
	dc.w	$00e8,$0005,$00ea,$3e80
	dc.w	$00ec,$0005,$00ee,$5dc0
	dc.w	$00f0,$0005,$00f2,$7d00,$0180
Lf112
	dc.w	$0000,$0182,$0f00,$0184,$0f00,$0186,$0b0b,$0188
	...
	dc.w	$0666,$01ba,$0555,$01bc,$0444,$01be,$0222,$9801,$ff00,$0180
Lf196
	dc.w	$0000,$0182,$0f00,$0184,$0f00,$0186
	...
	dc.w	$0888,$01b8,$0666,$01ba,$0555,$01bc,$0444,$01be,$0222,$ffff,$fffe

titleCopper
;Lf218
	dc.w	$0100,$5200,$0104,$0000
	dc.w	$008e,$1671,$0090,$06d1,$0092,$0030,$0094,$00d8,$0102,$0000,$0108,$0000,$010a,$0000
titleCopperPlanes
	dc.w	$00e0,$0005,$00e2,$42da
	dc.w	$00e4,$0005,$00e6,$6c1a
	dc.w	$00e8,$0005,$00ea,$955a
	dc.w	$00ec,$0005,$00ee,$be9a
	dc.w	$00f0,$0005,$00f2,$e7da
	dc.w	$0120,$0000,$0122,$0000,$0124,$0000,$0126,$0000,$0128,$0000,$012a,$0000,$012c,$0000,$012e
	...
	dc.w	$038a,$01a4,$0068,$01a6,$0057,$ffff,$fffe

Lf430
	dc.w	$0100,$5200,$0104,$0000,$008e,$1571,$0090,$05d1,$0092
	dc.w	$0030,$0094,$00d8,$0102,$0000,$0108,$0000,$010a,$0000,$00e0
Lf456
	dc.w	$0005,$00e2,$0000,$00e4,$0005,$00e6
	...
	dc.w	$0000,$0134,$0000,$0136,$0000,$0138,$0000,$013a,$0000,$013c,$0000,$013e,$0000,$0180
Lf4be
	dc.w	$0000,$0182
...
Lf4f2
	...
	dc.w	$0333,$01bc,$0222,$01be,$0111,$3001
Lf53e
	dc.w	$ff00,$019a,$0000,$3101,$ff00,$019a,$0000,$3201,$ff00,$019a
...
Lf632
	dc.w	$0e10,$ff01,$ff00,$009c,$8010,$ffff,$fffe

Lf640	dc.w	$0100,$5200,$0104,$0000,$008e,$1671,$0090,$06d1,$0092
	dc.w	$0030,$0094,$00d8,$0102,$0000,$0108,$0000,$010a,$0000,$00e0
Lf666	dc.w	$0005,$00e2,$42da,$00e4,$0005,$00e6
	...
	dc.w	$ff00,$009c,$8010,$ffff,$fffe

Lf7fc
	dc.w	$0100,$5200,$0104,$0040,$008e,$1571,$0090,$f9d1,$0092,$0030,$0094
...
Lfe06
	...
	ext.w	d0
	lea	Lfe5a,a0
	move.b	(a0,d0.w),Lfde4		ASCII key code (when key released)
Lfe32
	moveq	#$50,d0
Lfe34
	dbra	d0,Lfe34
...
Lfe5a
	dc.b	'`1234567890-=\',$00,$00
	dc.b	'qwertyuiop[]',$00,'123'
	dc.b	'asdfghjkl;''',$0d,$00,'456'
	dc.b	$00,'zxcvbnm,./',$00,$00,'789'
	dc.b	' ',$0a,$00,$0d,$0d,$00,$0a,$00,$00,$00,'-',$00,$00,$00,$00,$0a		; backspace, delete and cursor left all give $0a
	ds.b	16	; function keys, help
	ds.b	16	; shift, caps lock, ctrl, alt, amiga keys
...
L103f6
	dc.w	$0000
L103f8
	dc.l	L16704,L16718,L1672c,L16740
L10408
	dc.w	$0000
L1040a
	dc.w	$0000
...
L105ac
	...
	bne.s	L10600
L105f0
	cmpi.w	#$21,player1Shield
	beq.s	L10600
	addq.w	#1,player1Shield
L10600
	tst.w	L74b6
	beq.s	L10620
	tst.w	L74b4
...
L11e40
	dc.b	' '
L11e41
	dc.b	'1 completed',$00,$00
L11e4e
	dc.b	$04,$04,'pods x ',$04
L11e58
	dc.b	'200',$00
L11e5c
	dc.b	$01,'sorry no bonus',$00
L11e6c
	dc.b	'100',$00,'200',$00,'400',$00,'800',$00
L11e7c
	dc.b	$0d,$8c,'P',$02,'player '
L11e87
	dc.b	'1 get ready',$00
L11e93
	dc.b	$0d,$a7,'n',$02,'player '
L11e9e
	dc.b	'1',$0d,$91,$09,'g a m e  o v e r',$00
L11eb3
	dc.b	$0d,$96,'a',$05,'men:',$00
L11ebc
	dc.b	$0d,'ts',$02,'w a r p  g a t e  ',$03,'created',$00
L11edb
	dc.b	$04,'      ',$00,$02,'please',$00,$02,'select',$00,$02,' play ',$00,$02,' mode ',$00,$04,'      ',$00,$04,'  use ',$00,$02,'  f1  ',$00,$02,'  '
	...
	dc.b	'    ',$00,$04,'      ',$00,$ff
L120ac
	dc.b	$0d,$99,$bd,$04,'current',$0d,$86,$05,'skill level: ',$02
L120c8
	dc.b	'1',$0d,$96,$06
L120cc
	dc.b	' normal ',$00
L120d5
	dc.b	'  slow   normal   fast  '
L120ed
	dc.b	$0d,$ae,$d7
L120f0
	dc.b	$04,'k',$0d,$ae,$06
...
L120fc
	dc.b	' normal ',$00
L12105
	dc.b	' normal def mode'
L12115
	dc.b	$0d,'<',$c2,$02,'        ',$00
L12122
	dc.b	$0d,'<',$c4,$02,'        ',$00
L1212f
	dc.b	$02,$0d,$0a,$00,'the beginning is a relatively fractal multiverse.',$03,$0d,$0a,$08,'know then tha'
	...
	dc.b	'right 1989',$04,'  visionary design technologies inc.',$00,$00
L12626
	dc.b	$0d,$82,$0e,'production a'
...
L12740
	dc.b	$0d,$a5,$0a,$04,'2 5 0 0',$00
L1274c
	dc.b	$0d,$a5,$0a,$03,'1 5 0 0',$00
L12758
	dc.b	$0d,'a',$7f,$01,'a ',$02,'mastercoda',$03,' experience'
	dc.b	$04,' mutation',$0d,'U',$07,$05,'realeyezed',$03,' bi ',$04,'s`ren ',$05,$27,$01,'sodan',$05,$27,' ',$02,'gr`nbech',$00


L127ae
	move.b	$bfec01,d0
	cmpi.b	#$4d,d0		F10 pressed
	beq	L127e6
	cmpi.b	#$77,d0		RETURN pressed
	bne.s	L127e2
	movea.l	gameDisplayMem,a0
	movea.l	#L12758,a1
	move.w	#0,d0
...
L127e6
	move.l	gameDisplayMem,-(a7)
	move.w	Led62,d0
	swap	d0
	move.w	Led66,d0
	move.l	d0,gameDisplayMem
	movea.l	#L1289c,a0
L12806
	subq.w	#1,L127e4
	tst.w	L127e4
...
L137b6
	dc.l	L76a00,L78800,L7a600,L7c400,L7e200
L137ca
	dc.l	L76a00
...
L138de
	dc.w	$0000
L138e0
	dc.w	$03ff,$ffff,$81ff,$ffff,$c0ff,$ffff,$e07f,$ffff,$f03f,$ffff,$f81f,$ffff,$fc0f,$ffff,$fe07,$ffff
	dc.w	$ff03,$ffff,$ff81,$ffff,$ffc0,$ffff,$ffe0,$7fff,$fff0,$3fff,$fff8,$1fff,$fffc,$0fff,$fffe,$07ff
...
alienBullets	; both collision and drawing
;L13c38
	eori.b	#$a,L13b86
	movea.l	#Lffea,a3
	movea.l	gameDisplayMem,a1
...
animatePortal	; portal is a hardware sprite
;L13d9c
	movea.l	#L13e24,a0
	addq.b	#1,L13d9b
	andi.b	#3,L13d9b
...
L14b3a
	move.b	(a0)+,(a1)+
	dbra	d0,L14b3a
	move.l	#L4f73a,gameDisplayMem
	jsr	L156ca
	move.l	#L5aaea,gameDisplayMem
	jsr	L156ca
	jsr	L14cdc
	jsr	Le77a
	move.l	#L71c00,Lfcf8
...
L14bec
	...
	moveq	#26,d0
	jsr	loadData
	movea.l	#Data26+DATA26SIZE,a0
	movea.l	#Data26,a1
	movea.l	#Data26End,a2
	jsr	decompress
	movea.l	#L59c40,a0
	movea.l	#Lf112,a1
	moveq	#$1f,d0
L14c4e
	...
	move.l	#$1f40,d3
	jsr	L137ce
	move.l	#specialThanksCopper,Lfcf8
	jsr	Lfcfc
	jsr	Lfef8
	jsr	Lff1e
	move.l	#$3d090,d7
...
L14dfc
	dc.w	$0000
L14dfe
	dc.l	L6a99a,L6cf1a,L6841a,L65e9a
...
L14ef2
	...
	dc.w	$0000,$08a0
L14f98
	dc.w	$0100,$5200,$0104,$0000,$008e,$2a71,$0090,$f2d1,$0092,$0030,$0094,$00d8,$0108,$0002
...
L1549e
	...
	move.l	L133a6,-(a7)
	clr.w	L133a4
	move.l	#L702aa,L13392
	tst.w	L15576
	beq.s	L15550
	move.w	#1,L133a4
	clr.w	L15576
	move.l	#L702aa,L133a6
L15550
	jsr	L133b4
	move.l	(a7)+,L133a6
	tst.w	L15574
...
L156bc
	dc.w	$021c
L156be
	dc.w	$fffe,$0006,$0002,$0002,$0006,$0002
...
L1576c
	move.l	#L6736e,L133a6
	move.w	#$30,$dff09a
	moveq	#3,d0
	jsr	loadData
	movea.l	#Data3+DATA3SIZE,a0
	movea.l	#Data3,a1
	movea.l	#Data3End,a2
	jsr	decompress
	clr.w	L138de
...
L158c8
	move.b	$bfec01,d0
	cmpi.b	#$76,d0		RETURN released
	beq.s	L15876
	move.b	d0,L15988
L158da
	cmpi.b	#$5f,d0		F1 pressed
	beq	L1598a
	cmpi.b	#$5c,d0		F2 released
	beq	L159ba
	cmpi.b	#$5b,d0		F3 pressed
	beq	L159f0
	cmpi.b	#$59,d0		F4 pressed
	beq	L15a1e
	cmpi.b	#$b3,d0		J pressed
	beq	L1595a
	cmpi.b	#$b1,d0		K pressed
	beq	L15966
	cmpi.b	#$bb,d0		D pressed
	beq	L15972
	cmpi.b	#$93,d0		N pressed
	beq	L1597e
	cmpi.b	#$fd,d0		1 pressed
	beq	L15936
	cmpi.b	#$fb,d0		2 pressed
	beq	L15942
	cmpi.b	#$f9,d0		3 pressed
	beq	L1594e
	bra	L15876
...
L15e48
	...
	dc.w	$09fc,$0afd,$0bfd,$0dfe,$0efe,$0fff
L15eb4
	dc.w	$0007,$0008,$0018,$0019,$0019,$002a,$002a,$003b,$004c,$004c
...
L15f72
	clr.w	destroyedAnimationFlag
	move.l	#L798ba,L133a6
	moveq	#27,d0
	jsr	loadData
	movea.l	#Data27+DATA27SIZE,a0
	movea.l	#Data27,a1
	movea.l	#Data27End,a2
	jsr	decompress
	move.l	#$671f,d0
	movea.l	#L5fc3a,a0
L15fae
...
L1604e
	...
	moveq	#12,d0
	jsr	loadData
	movea.l	#Data12+DATA12SIZE,a0
	movea.l	#Data12,a1
	movea.l	#Data12End,a2
	jsr	decompress
	move.l	gameDisplayMem,Data12
...
L161d2
	clr.w	pilotAnimationFlag
	move.l	#L798ba,L133a6
	moveq	#10,d0
	jsr	loadData
	movea.l	#Data9+DATA9SIZE,a0
	movea.l	#Data9,a1
	movea.l	#Data9End,a2
	jsr	decompress
	movea.l	#Data10+DATA10SIZE,a0
	movea.l	#Data10,a1
	movea.l	#Data10End,a2
	jsr	decompress
	move.l	#$671f,d0
	movea.l	#L5fc3a,a0
L16226
...
L16380
	dc.w	$0000
L16382
	dc.w	$0001,$0001,$0001,$0001,$0001,$0001,$0001,$0001,$0001,$0002,$0002,$0004,$0001,$0001,$0001
...
L1647a
	...
	moveq	#11,d0
	jsr	loadData
	movea.l	#Data11+DATA11SIZE,a0
	movea.l	#Data11,a1
	movea.l	#Data11End,a2
	jsr	decompress
	lea	$dff000,a6
	movea.l	#Data4,a0
	move.w	#$f9,d0
...
L165aa
	dc.w	$0000
L165ac
	dc.w	$0100,$5200,$0104,$0000,$008e,$1671,$0090,$06d1,$0092,$0030,$0094,$00d8,$0102,$0000,$0108
	dc.w	$0000,$010a,$0000,$00e0
L165d2
	dc.w	$0005,$00e2,$0000,$00e4,$0005,$00e6,$2940,$00e8,$0005,$00ea,$5280,$00ec
	...
	dc.w	$0000,$013a,$0000,$013c,$0000,$013e,$0000,$0180
L1663a
	dc.w	$0000,$0182,$0fff,$0184,$0f00,$0186,$0fde,$0188
...
L166c8	dc.b	'0000000000'
L166d2	dc.b	'0000000010'
L166dc	dc.b	'0000000025'
L166e6	dc.b	'0000000050'
L166f0	dc.b	'0000000070'
L166fa	dc.b	'0000000075'
L16704	dc.b	'0000000100'
L1670e	dc.b	'0000000150'
L16718	dc.b	'0000000200'
L16722	dc.b	'0000000250'
L1672c	dc.b	'0000000400'
L16736	dc.b	'0000000500'
L16740	dc.b	'0000000800'
L1674a	dc.b	'0000001500'
L16754	dc.b	'0000002500'
...
L1700c
	dc.b	'SODA'
L17010
	dc.b	$0d,'Y',$01,'t h e  i m m o r t a l : '
L1702c
	dc.b	'-'
L1702d
	dc.b	'-'
...
L17058
	...
	dc.b	'00000000',$01,$0d,'O',$07,'--- 0000000000',$00
L17109
	dc.b	$0d,$d1,'!--- 0000000000',$01,$0d,$d1,$07,'--- 0000000000',$01,$0d
	...
	dc.b	'--- '
L171b2
	dc.b	'0000000000',$00,$00,'SODA'
L171c2
	dc.b	$0d,'7! 1.',$01,$0d,'7',$07,' 2.',$01,$0d,'7',$07,' 3.',$01,$0d,'7',$07,' 4.',$01,$0d,'7',$07,' 5.',$01,$0d,'7',$07,' 6.',$01,$0d,'7'
	dc.b	$07,' 7.',$01,$0d,'7',$07,' 8.',$01,$0d,'7',$07,' 9.',$01,$0d,'7',$07,'10.',$00
L17208
	dc.b	$0d,$b9,'!11.',$01,$0d,$b9,$07,'12.',$01,$0d,$b9,$07,'13.',$01,$0d,$b9,$07,'14.',$01,$0d,$b9,$07,'15.',$01,$0d,$b9,$07
	dc.b	'16.',$01,$0d,$b9,$07,'17.',$01,$0d,$b9,$07,'18.',$01,$0d,$b9,$07,'19.',$01,$0d,$b9,$07,'20.',$00
L1724e
	dc.b	$0d,'D',$18
L17251
	dc.b	'        t h e  m o r t a l s        ',$00
L17276
	dc.b	'player 1, please enter your initials'
L1729a
	dc.b	'player 2, please enter your initials'
L172be
	dc.b	'        t h e  m o r t a l s        '
L172e2
	dc.b	'           and winner text          '
L17306
	dc.b	'   please enter your team initials  '
L1732a
	dc.b	$00
L1732b
	dc.b	$00
...
L1732e
	...
	beq.s	L173a2
	clr.w	L1732c
	cmpi.b	#$d,d0		RETURN / ENTER
	beq	L1746a
	cmpi.b	#$a,d0		BACKSPACE / DELETE / CURSOR LEFT
	beq.s	L173aa
	move.w	L17514,d0
	cmp.w	L1751a,d0
	beq.s	L173a2
...
L175ee
	...
	move.w	#0,$dff066
	move.w	#0,$dff064
	move.l	#L6eb70,$dff050
	move.l	radarDisplayMem,$dff054
	move.w	#$9396,$dff058
	rts
...
L178ea
	dc.b	$0d,$2f,$5f,$02,'fatal diskette error -',$03,' please reboot!',$00,$00
...
* lists of audio information, one per audio channel
L1af4a
	dc.w	$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000
L1af66
	dc.w	$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000
L1af82
	dc.w	$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000
...
* list of sound effects (numbered 0 to 34) (not sure 0 is used or valid though)
L1afba
	dc.l	L50000,$000006ae,$00000000,$0000007c,$00000001,$00000001,$00000000,$00000000,$00000000,$00000000
	dc.l	L1f6fc,$0000050a,$0000000d,$000001a2,$00000009,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L1ca00,$00002cfc,$00000016,$000001e8,$00000001,$ffffffff,$00000000,$00000000,$00000000,$00000000
	dc.l	L1ca00,$0000ae60,$00000040,$000001ac,$00000001,$00000001,$00000000,$00000000,$00000000,$00000000
	dc.l	L27860,$00005a40,$00000028,$000001ac,$00000001,$00000001,$00000005,$00000000,$00000000,$00000000
	dc.l	L28fa0,$00004300,$00000028,$000001ac,$00000001,$00000003,$00000000,$00000000,$00000000,$00000000
	dc.l	L1fc06,$00003be0,$0000001e,$00000212,$00000003,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L36412,$00006666,$00000001,$00000226,$00000002,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L237e6,$00001ac2,$0000001e,$0000015e,$00000002,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L252a8,$0000186c,$00000036,$000001d2,$00000008,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L1ca00,$0000ae60,$00000040,$000001b7,$00000001,$00000001,$00000000,$00000000,$00000000,$00000000
	dc.l	L26b14,$000061a0,$00000040,$000001f4,$00000001,$00000001,$00000000,$00000000,$00000000,$00000000
	dc.l	L2ccb4,$000019ce,$0000002d,$00000166,$00000003,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L2e682,$00000de0,$0000003c,$0000010e,$00000005,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L1f6fc,$0000050a,$0000002d,$000000e6,$00000007,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L2f462,$00000670,$00000032,$000001ac,$00000004,$00000001,$00000000,$00000000,$00000000,$00000000
	dc.l	L2fad2,$00000670,$00000032,$000001ac,$00000004,$00000001,$00000000,$00000000,$00000000,$00000000
	dc.l	L30142,$000027fe
L1b26a
	dc.l	$00000032,$000000f5,$00000003,$00000001,$00000011,$00000000,$00000000,$00000001
	dc.l	L32940,$00002e70,$00000040,$0000021c,$00000002,$00000001,$00000000,$00000000,$00000000,$00000000
	dc.l	L357b0,$000020ae,$00000040,$000000b4,$00000003,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L2e682,$00000de0,$0000003c,$00000118,$00000005,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L3785e,$000009dc
L1b30a
	dc.l	$00000032,$000001ac,$00000003,$00000001,$00000015,$00000000,$00000000,$00000001
	dc.l	L3823a,$000019f4,$00000040,$00000226,$00000005,$00000001,$00000000,$00000000,$00000000,$00000000
	dc.l	L252a8,$0000186c,$00000040,$0000024a,$00000007,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L3f71c,$00002540
L1b382
	dc.l	$00000040,$000001ac,$00000003,$00000001,$00000018,$00000000,$00000000,$00000001
	dc.l	L3785e,$000009dc
L1b3aa
	dc.l	$00000032,$00000152,$00000003,$00000001,$00000019,$00000000,$00000000,$00000001
	dc.l	L39c2e,$0000201c,$00000028,$00000168,$00000005,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L3bc4a,$00001bc8,$00000040,$000000f0,$00000004,$00000001,$00000000,$00000000,$00000000,$00000000
	dc.l	L3d812,$00001f0a,$00000032,$00000154,$00000005,$00000001,$00000000,$00000000,$00000000,$00000000
	dc.l	L41c5c,$000025ca,$00000040,$000002f4,$00000002,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L44228,$000005a0,$00000040,$000000d6,$00000006,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L41c5c,$000025ca,$00000036,$000001f2,$00000006,$00000001,$00000000,$00000000,$00000000,$00000001
	dc.l	L1c764,$000001f2,$00000040,$000003de,$00000001,$00000063,$00000000,$00000000,$00000000,$00000001
	dc.l	L1c764,$00000ca4,$00000040,$000000f0,$00000001,$0000000b,$00000000,$fffffffc,$00000023,$00000001
	dc.l	L358d0,$00000146,$00000023,$0000012c,$00000005,$00000008,$00000000,$00000000,$00000032,$00000001
* end of sound effect list
...
* Note about 'Data' labels:
* The start addresses (e.g. Data0) are of both the compressed and decompressed data
* The end addresses (e.g. Data0End) are of the decompressed data
...
L1c456
	...
	dc.b	'- 0000000000',$01,$0d,$d1,$07,'--- 0000000000',$01,$0d,$d1,$07,'--- 0000000000',$00,$00,'SODA'

L1c60c
	dc.w	$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000
	dc.w	$000a,$0014,$001e,$0028,$0032,$003c,$0046,$0050,$0064,$006e,$0078,$0082,$008c,$0096,$00a0,$00aa
...
* code added by DV to set bitplane pointers in the copper lists used for vertical shrinking of the ship animations
...
.loop	move.w	d7,6(a0)
	...
	dbra	d5,.loop
	rts
* end of code added by DV


* original program decompressed Data12 to give non-relocatable version of this code (and data)
L50004	bsr	setShrinkCoppers	; added by DV
	jsr	L58454
	jmp	L585fe

* Data50010.s is here in the original program
...
L58772
	...
	tst.w	L5875c
	beq	L587cc
	move.l	#L53158,L5875e
	jsr	L5889e
	jsr	L58802
	move.l	#L530fc,L58970
L587b6
	move.b	$dff006,d0
	cmpi.b	#-$6,d0
	bne	L587b6
...
L587cc
	move.l	#L55ae4,L5875e
	jsr	L588e2
	jsr	L58802
	move.l	#L55ac8,L58970
L587ec
	move.b	$dff006,d0
	cmpi.b	#-$6,d0
	bne	L587ec
...
L5889e
	jsr	L58980
	move.l	#L50790,$dff050
	move.l	#L53158,$dff054
	move.l	#$ffffffff,$dff044
	move.l	#$9f00000,$dff040
	clr.l	$dff064
	move.w	#$3c16,$dff058
