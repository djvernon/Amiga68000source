...
A.30000
	...
	dc.b	$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$53,$00,$02,$ff,$e0
	dc.b	$00,$1f,$00,$04,$ff,$e0,$ff,$e1,$00,$1e,$00,$1f,$00,$04,$ff,$e0
	dc.b	$ff,$e1,$00,$1e,$00,$1f,$00,$04,$ff,$e0,$ff,$e1,$00,$1e,$00,$1f
	dc.b	$00,$04,$ff,$e0,$ff,$e1,$00,$1e,$00,$1f,$00,$04,$ff,$e0,$ff,$e1
...
l00003d	lea	$dff000,a0
	move.l	#A.31a28,cop1lch(a0)
...
l000092	move.w	d0,(a2)+
	...
	move.w	#$2481,diwstrt(a0)
	move.w	#$2cc1,diwstop(a0)
	move.l	#$a156,cop1lch(a0)
	move.w	copjmp1(a0),d0
	move.w	#$87f0,dmacon(a0)
	rts
...
l000093	lea	$dff000,a0
	move.w	#$7fff,adkcon(a0)
	move.w	#$8210,dmacon(a0)
	move.w	#$8100,adkcon(a0)
	bsr	l00009c
	bsr	l0000aa
	bsr	l00009e
	rts

l000094	lea	$dff000,a0
	move.w	#$7fff,adkcon(a0)
	move.w	#$8210,dmacon(a0)
	move.w	#$8100,adkcon(a0)
	rts
...
l00009a	move.w	#3874,$dff000+color0
	move.l	A.31fac,a1
	sub.w	#1,A.31fb8
	bne.s	l000098
	move.w	#1000,d1

l00009b	move.w	d0,$dff000+color0
	addq.w	#3,d0
	btst	#6,$bfe001
	beq.s	l000099
	dbra	d1,l00009b
...
l00009c	move.b	#127,$bfd100
	move.b	#111,$bfd100
	move.w	#60000,d0

l00009d	dbra	d0,l00009d
	rts

l00009e	move.b	#255,$bfd100
	move.b	#239,$bfd100
	move.b	#255,$bfd100
	rts

l00009f	move.w	A.31f98,d1
	move.w	d1,d2
	and.w	#1,d2
	lsl.w	#2,d2
	or.b	#107,d2
...
l0000a9	dbra	d0,l0000a9
	rts


l0000aa	move.b	$bfe001,d0
	btst	#4,d0
	beq.s	l0000ab
	bsr	l0000a4
	bra	l0000aa

l0000ab	clr.w	A.31f98
	rts


l0000ac	lea	$dff000,a0
	move.l	A.31fb0,dskpth(a0)
	move.w	#$4000,dsklen(a0)
	move.w	#$8400,adkcon(a0)
	move.w	A.31fa0,dsksync(a0)
	move.w	#$9778,dsklen(a0)
	move.w	#$9778,dsklen(a0)
	move.w	#2,intreq(a0)

l0000ad	move.w	intreqr(a0),d0
	and.w	#2,d0
	beq.s	l0000ad
...
l0000ae	move.l	A.31fb0,dskpth(a0)
	move.w	#$4000,dsklen(a0)
	move.w	#$400,adkcon(a0)
	move.w	#$d9c8,dsklen(a0)
	move.w	#$d9c8,dsklen(a0)
	move.w	#2,intreq(a0)

l0000af	move.w	intreqr(a0),d0
	and.w	#2,d0
	beq.s	l0000af
...
l0000b4	move.l	A.31fb0,a2
	move.w	#500-1,d0

l0000b5	move.w	#$aaaa,(a2)+
	dbra	d0,l0000b5
...
l0000b7	move.w	(a1)+,d0

l0000b8	move.w	d0,d1
	swap	d1
	lsr.l	#1,d1
	and.l	#$5555,d0
	and.l	#$55550000,d1
