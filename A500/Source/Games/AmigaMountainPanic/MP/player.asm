...
playWalkSound:
;{
    ;AND8 #%100			; 23/08/2021 original BBC code
    AND8 #%101			; 12/10/2021 don't play at odd positions, to more closely match BBC on the Amiga
    BNE skipWalkSound ; should this be beq?

    ; Play walk sound effect
    LDA playerWalkSfxFlag
	eor.b	#5,AREG ; pitch offset
...
;    STA notereq+3
	...
	tst.b	playerWalkSfxFlag
	bne.s	.hi

.lo	lea	walkSoundLoInfo,a1
	bra.s	.play

.hi	lea	walkSoundHiInfo,a1
...
;	STA jumpToUpdate+2
	moveq	#0,d3
	move.b	playerState,d3
	add.w	d3,d3
	lea	playerStateJumpTables,a5
	move.l	(a5,d3.w),a5
...
.checkEnemies:        
	JSR checkPlayerColl ; Returns with N set if collision
	BPL .noCollSolange
...
.redrawPlayer
	IFD	NOT_REQUIRED
	;
	; DJV this section not required because Amiga bob system takes care of screen restores for player
	;
...
.ok1
	...
	TXA
	CMP8 #$58
	BCC .ok ; last row, no need for any more		;Changed from BCS as 68000 differs from 6502
	ADC #8
	INY
	STA playerTileList,Y
.ok2
...
.checkHealth:
	...
	LDA playerEnergy
	CMP8 #28-8
	BCC .pippin		;Changed from BCS as 68000 differs from 6502
	LDA playerEnergy
	CLC
	ADC #8
	STA playerEnergy
...
;	STA (curScreenLO),Y
;DJV	LDY t0
	move.l	curScreen(pc),a5
	and.b	#~SCREEN_FLAGS_ITEM_PRESENT,2(a5)
...
updatePlayerBob
	...
	CPX #4
	BCC .noAnimOnThisState		;Changed from BCS as 68000 differs from 6502
...
;	STA t1
	...
	lea	bobDataTable(pc),a2
	moveq	#0,d6
	moveq	#0,d7
	move.b	AREG,d6
	move.w	d6,SourceCol(a2)
	move.w	#PLAYER_ROW,SourceRow(a2)

	move.b	playerPosX,d6
	move.b	playerPosY,d7
...
;	{
	LDA playerPosY
	CMP8 #$D0
	BCS .notAtBottom		;Changed from BCC as 68000 differs from 6502
...
.notAtBottom	
	...
	BNE .hitFloor
        ...
        ; 03/03/2012 : Player drop speed is 4 pixels;	DJV 2 pixels on Amiga
        LDA playerPosY
	TAX
	AND8 #1
	BNE .onlyAdd1
...
.ropeGoingRight
	...
	JSR getTile
	LDA collData,Y
	AND8 #(_bitCollidable|_bitClimbable)
...
;	BIT t0
	...
	JSR getTile
	LDA collData,Y
	AND8 #(_bitHookable|_bitCollidable)
	CMP8 #(_bitHookable|_bitCollidable)
	BNE .upOut
...
;	BIT t0
	...
	LDA playerPosY
	CMP8 #$22
	BCC .notAtTop		;Changed from BCS as 68000 differs from 6502
	LDA #_screenChangeUp
	JSR checkForScreenChange
	LDX #0
	RTS
...
.notAtTop:
	...
	LDA collData,Y
	TAX
	AND8 #(_bitCollidable)	; OR _bitRope)
	BEQ .step2
	LDX #1
	RTS
.step2
...
;	{
	move.l	curScreen(pc),a5
	CMP8 #_screenChangeDown
	BEQ .down
	
	CMP8 #_screenChangeRight
	BEQ .right

	CMP8 #_screenChangeUp
	BEQ .up
...
.out:
	...
	STA ropeState
	JSR drawScreen
;	bsr	saveScreenUnderBobs	;TODO not sure this needed now?
