...
* X  : centering flag
* Y  : string index
* d4.w = dest column number (NB column width is 8 pixels)
* d5.w = dest y
...
.calcStringLengthLooop:
	tst.b	(a1)+
	bne.s	.calcStringLengthLooop
	move.l	a1,d3
	sub.l	a0,d3
	subq.l	#1,d3		exclude null terminator

	; numSpaces = (14-stringlength) / 2
.calcNumSpaces:
	move.w	#28,d6		doubled was 14 on BBC Micro
	sub.w	d3,d6
	lsr.w	#1,d6
	add.w	d6,d4		add to dest column number
