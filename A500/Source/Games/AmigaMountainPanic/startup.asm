...
; DJV 15/07/2014.  Disabled the setting of very high task priority below, because it prevented the Input Handler from working after the program had been run once
        move.l  a2,a1
...
;DJV 17/07/2014 can't Forbid as Input Handler stops working
;        move.l  4.w,a6
;        jsr		_LVOForbid(a6)
...
;	dc.b	NT_INTERRUPT,127	; LN_TYPE, LN_PRI ** highest priority ** 
	dc.b	NT_INTERRUPT,52		; LN_TYPE, LN_PRI
	dc.l	ih_name	; LN_NAME
	dc.l	0		; IS_DATA
	dc.l	.ih_code	; IS_CODE

*
* Input Handler modified by DJV (based on ReadKeyboard.s)
* Following also added to SysDefines.i :-
...
;	move.l	ie_NextEvent(a0),a0		get pointer to next event in list
	move.l	(a0),a0				get pointer to next event in list (optimised above as ie_NextEvent = 0)
	move.l	a0,d1
	bne.b	.loop

	move.l	a0,d0			return end of event list (i.e. 0), so that no events are passed on (this removes the need to set ie_Class to IECLASS_NULL above)
	rts
...
DebugWithMonam	dc.b	0	;Added by DJV
RawKeyCode	dc.b	0	;Added by DJV
RawKeyArray	ds.b	128	;Added by DJV
