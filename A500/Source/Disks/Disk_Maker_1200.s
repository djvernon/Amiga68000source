	section	Disk_Maker,code_c


load.save.attempts	equ	$1f4
...
BOOT_STACK	equ	$7cc00
BOOT_SPACE	equ	$7fc00


PROGRAM_SPACE	equ	$400
...
boot.start
	...
	lea	BOOT_STACK,sp		stack pointer
	lea	custom,a6		base of custom chips
	move.w	#$01ff,dmacon(a6)	disable all DMA
	move.w	#$7fff,intena(a6)	disable all interrupts
	clr.w	color0(a6)
	...
	lea	BOOT_SPACE,a1		destination
	move.w	#256-1,d0		1024 bytes
copy.loop2
	move.l	(a0)+,(a1)+
	dbra	d0,copy.loop2
	jmp	BOOT_SPACE		jump to main code


boot.main
	move.w	#$8210,dmacon(a6)	set dmaen, dsken
...
load.program
	moveq	#27,d0			start at track 27
	moveq	#29,d1			29 tracks
	lea	PROGRAM_SPACE,a0	destination
	bsr.s	b.load
	bmi.s	load.program
	jmp	PROGRAM_SPACE
...
b.read.track
	...
	move.l	#$7cc00,dskpth(a6)	destination
...
next.colour
	...
	dc.w	0,2,2			blank, insert dest., continue
...
write.bottom.side
	bsr	display.3.messages
	dc.w	1,0,0			writing dest., blank, blank

	lea	beginning(pc),a0	source
	lea	boot.block.space(pc),a1	destination
	bsr	make.boot.block
	...
	lea	boot.block.space(pc),a5	source
	...
	moveq	#27,d0			start at track 27
	moveq	#29,d1			29 tracks
	lea	program.data+32,a0	source
	bsr	save
	bpl.s	bottom.side.written

bottom.side.not.written
	bsr	display.3.messages
	dc.w	0,1,1			blank, error writing, retry
...
bottom.side.written
	...
	dc.w	2,3,0			complete, reset, blank
...
setup.screen
	lea	screen.space,a0
	lea	32000(a0),a0		end of screen area
	moveq	#0,d0
	moveq	#0,d1
	moveq	#0,d2
	moveq	#0,d3
...
.loop	movem.l	d0-d3,-(a0)		clear the screen
	...
	lea	screen.space+120,a0	start of screen, bitplane 4
	moveq	#120,d6
...
.loop	move.w	d1,(a0)+		save middle words
	...
	add.w	d6,a0			to next line of bitplane 4
	dbra	d4,print.lines
	rts
...
print.char
	lea	screen.space,a5		start of screen, bitplane 1
	move.w	d2,d3			y
	lsl.w	#5,d3			y*32
	move.w	d3,d5
	add.w	d3,d3			y*64
...
code.custom.track			; a5 = source
	lea	track.space(pc),a2	destination
	move.w	#6560/2-1,d0
	move.l	#$aaaaaaaa,d1
...
.loop	move.l	d1,(a2)+		save data for track gap
	...
	lea	track.space+1140(pc),a2
	move.l	#$44894489,4(a2)	two sync markers

	moveq	#-1,d0
	move.b	(head.position+1).w,d0
	add.b	d0,d0
	add.b	(top.head+1).w,d0	track number
...
title.text
	dc.b	5,5,3,10,8,'Daniel''s Disk Maker'
	dc.b	3,14,24,'Version 0.9',0
...
writing.text
	dc.b	1,8,'Writing Destination Disk',0

finished1.text
	dc.b	1,12,'Process Complete',0

position2.text
	dc.b	2,120,5,1,0

error.text
	dc.b	1,5,'Error Writing Destination Disk',0

insert.text
	dc.b	1,8,'Insert Destination Disk',0

finished2.text
	dc.b	1,5,'Reset Computer To Load Program',0
...
boot.block.space	ds.l	256

track.space	ds.l	6560


	section	screen,code_c

screen.space	ds.l	8000


	section	program,code_c

program.data	incbin	3D_example_music
