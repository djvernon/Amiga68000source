...
;	Disassembly of Carrier Command backup routine
...
boot.start
	...
	lea	$7cc00,sp		stack pointer
	lea	custom,a6		base of custom chips
	move.w	#$01ff,dmacon(a6)	disable all DMA
	move.w	#$7fff,intena(a6)	disable all interrupts

	lea	boot.main(pc),a0	source
	lea	$7fc00,a1		destination
	move.w	#256-1,d0		1024 bytes
copy.loop2
	move.l	(a0)+,(a1)+
	dbra	d0,copy.loop2
	jmp	$7fc00			jump to main code
...
setup.screen
	lea	$7fc00,a0		end of screen area
	moveq	#0,d0
	moveq	#0,d1
	moveq	#0,d2
	moveq	#0,d3
...
.loop	movem.l	d0-d3,-(a0)		clear the screen
	...
	lea	$77f78,a0		start of screen, bitplane 4
	moveq	#120,d6
...
print.char
	lea	$77f00,a5		start of screen, bitplane 1
	move.w	d2,d3			y
	lsl.w	#5,d3			y*32
	move.w	d3,d5
	add.w	d3,d3			y*64
...
text2.pointers
	dc.w	blank.text-backup.start+$800
	dc.w	error.text-backup.start+$800
	dc.w	insert.text-backup.start+$800
